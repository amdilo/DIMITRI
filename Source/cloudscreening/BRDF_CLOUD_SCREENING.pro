...
;*                                      UPDATE DATABASE AND NCDF INGESTION INPUT FILES WITH BRDF RESULT VALUES
;*                                      CHANGE UNITS OF WVAP AND O3 FROM NCDF VALUES TO USED VALUES IN BRDF PROCESS
;*
;* VALIDATION HISTORY:
...
; AND SOME NUMERICAL CONSTANTS
  ...
  TOL       = 0.000005 ; TOLERANCE FOR FLOAT COMPARISON
  REF_O3    = 300.0
  REF_WV    = 2.0
...
; RETRIEVE WAVELENGTHS
  SENSOR_INFO = GET_SENSOR_BAND_INFO(SENSOR, VERBOSE=VERBOSE)
  NB_BANDS    = SENSOR_INFO.NB_BAND
  WAVELENGTHS = SENSOR_INFO.BAND_WAVELENGTH
...
; DEFINE INPUT/OUTPUT FILES AND DIRECTORIES
  ...
  INGESTION_FOLDER  = GET_DIMITRI_LOCATION('INGESTION_OUTPUT')
...
; SELECT CLOUD TRAINING DATA UPON ROI CRITERIA
  ...
  RES = WHERE(STRCMP(DB_DATA_CT.SITE_NAME,REGION)               EQ 1 AND $
              ...
              DB_DATA_CT.ROI_PIX_NUM GE PX_THRESH, NB_FILES)
...
;  RESTORE, IFILE
  ...
  ;----------------------
  ; GET FILES FROM INGESTION OUTPUT FOLDER ACROSS SELECTED YEARS
  
  YEAR_START = FIX(DECIMAL_YEAR_START)
  YEAR_STOP = FIX(DECIMAL_YEAR_STOP)
  
  YEAR_LIST = [ YEAR_START ]
  FOR YEAR=YEAR_START+1, YEAR_STOP DO BEGIN
    YEAR_LIST = [ YEAR_LIST, YEAR ]
  ENDFOR
  
  DIR_FILES = INGESTION_FOLDER + DL + 'Site_' + REGION + DL + SENSOR + DL + 'Proc_' + PROC_VER + DL + STRTRIM(STRING(YEAR_LIST),2)
  PATTERN = REGION + '_' + SENSOR + '_' + PROC_VER + '*.nc'
  FILE_RESULT = FILE_SEARCH(DIR_FILES, PATTERN , COUNT=NB_FILES_SEARCH)
  ...
  SENSOR_L1B_REF = MAKE_ARRAY(NUM_NON_REF+2*NB_BANDS, NB_FILES_SEARCH*NB_DIRECTIONS, /DOUBLE, VALUE=MISSING_VALUE_FLT)

  ;----------------------
  ; LOOP OVER NCDF FILE IN PROCESSING DATASET TO FILL DATASET TABLE

  FOR IDX_FILE=0, NB_FILES_SEARCH-1 DO BEGIN
    ...
    ;----------------------
    ; GET ACQUISITION DATE FROM NCDF FILE AND COMPUTE DECIMAL YEAR
    
    CUR_NCDF_DATE = STRTRIM(GET_NCDF_GLOBAL_ATT(NCDF_FILENAME, 'ACQUISITION_DATE'))      
    IF KEYWORD_SET(VERBOSE) THEN BEGIN
      PRINT, '[' + STRTRIM(STRING(IDX_FILE+1),1) + '/' + STRTRIM(STRING(NB_FILES_SEARCH),1) + '] > ' + FCT_NAME + ' ' + CUR_NCFILE + ' :: ' + CUR_NCDF_DATE
    ENDIF
    
    NCDF_YEAR=FIX(STRMID(CUR_NCDF_DATE,0,4))
    NCDF_MONTH=FIX(STRMID(CUR_NCDF_DATE,5,2))
    NCDF_DAY=FIX(STRMID(CUR_NCDF_DATE,8,2))
    NCDF_HOURS=FIX(STRMID(CUR_NCDF_DATE,11,2))
    NCDF_MINUTES=FIX(STRMID(CUR_NCDF_DATE,14,2))   
    NCDF_SECONDS=FIX(STRMID(CUR_NCDF_DATE,17,2))
    
    DECIMAL_YEAR = DATETIME_TO_DECIMAL_YEAR(NCDF_YEAR,NCDF_MONTH,NCDF_DAY,NCDF_HOURS,NCDF_MINUTES,NCDF_SECONDS,VERBOSE=VERBOSE)
...
;      IF NCDF_INGEST_STRUCT.VARIABLES.ROI_STATUS[NUM_DIR] EQ 0 OR COUNT EQ 0 THEN BEGIN
      ...
      SENSOR_L1B_REF[0, IDX_STRUCT] = DECIMAL_YEAR
;      SENSOR_L1B_REF[1, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.VZA[IDX_CLEAR,NUM_DIR])
;      SENSOR_L1B_REF[2, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.VAA[IDX_CLEAR,NUM_DIR])
;      SENSOR_L1B_REF[3, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.SZA[IDX_CLEAR,NUM_DIR])
;      SENSOR_L1B_REF[4, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.SAA[IDX_CLEAR,NUM_DIR])
      SENSOR_L1B_REF[1, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.VZA[*,NUM_DIR])
      SENSOR_L1B_REF[2, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.VAA[*,NUM_DIR])
      SENSOR_L1B_REF[3, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.SZA[*,NUM_DIR])
      SENSOR_L1B_REF[4, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.SAA[*,NUM_DIR])

      ; LOOP ON VALID BANDS
      FOR BAND=0, NB_BANDS-1 DO BEGIN
;        RTOA = NCDF_INGEST_STRUCT.VARIABLES.REFL_BAND[IDX_CLEAR, NUM_DIR, BAND]
        RTOA = NCDF_INGEST_STRUCT.VARIABLES.REFL_BAND[*, NUM_DIR, BAND]
        IDX = WHERE(RTOA NE MISSING_VALUE_FLT, COUNT)
        IF COUNT NE 0 THEN SENSOR_L1B_REF[NUM_NON_REF+BAND, IDX_STRUCT] = MEAN(RTOA(IDX))
        IF COUNT GT 2 THEN SENSOR_L1B_REF[NUM_NON_REF+NB_BANDS+BAND, IDX_STRUCT] = STDDEV(RTOA(IDX))
      ENDFOR
      
;      AUXDATA = NCDF_INGEST_STRUCT.VARIABLES.ERA_OZONE[IDX_CLEAR,NUM_DIR]
      AUXDATA = NCDF_INGEST_STRUCT.VARIABLES.ERA_OZONE[*,NUM_DIR]
      IDX = WHERE(AUXDATA NE MISSING_VALUE_FLT, COUNT)
      IF COUNT NE 0 THEN SENSOR_L1B_REF[5, IDX_STRUCT] = MEAN(AUXDATA(IDX))
      IF COUNT GT 2 THEN SENSOR_L1B_REF[6, IDX_STRUCT] = STDDEV(AUXDATA(IDX))
      
;      AUXDATA = NCDF_INGEST_STRUCT.VARIABLES.ERA_PRESSURE[IDX_CLEAR,NUM_DIR]
      AUXDATA = NCDF_INGEST_STRUCT.VARIABLES.ERA_PRESSURE[*,NUM_DIR]
      IDX = WHERE(AUXDATA NE MISSING_VALUE_FLT, COUNT)
      IF COUNT NE 0 THEN SENSOR_L1B_REF[7, IDX_STRUCT] = MEAN(AUXDATA(IDX))
      IF COUNT GT 2 THEN SENSOR_L1B_REF[8, IDX_STRUCT] = STDDEV(AUXDATA(IDX))
      
      ; NO INFORMATION AVAILABLE FOR AUXDATA VARIABLES: VAR_RHUMMU, VAR_RHUMSD, VAR_ZONALMU, VAR_ZONALSD, VAR_MERIDMU, VAR_MERIDSD
      ; SET TO -999.0
      SENSOR_L1B_REF[9:14, IDX_STRUCT] = MISSING_VALUE_FLT
      
;      AUXDATA = NCDF_INGEST_STRUCT.VARIABLES.ERA_WATERVAPOUR[IDX_CLEAR,NUM_DIR]
      AUXDATA = NCDF_INGEST_STRUCT.VARIABLES.ERA_WATERVAPOUR[*,NUM_DIR]
      IDX = WHERE(AUXDATA NE MISSING_VALUE_FLT, COUNT)
      IF COUNT NE 0 THEN SENSOR_L1B_REF[15, IDX_STRUCT] = MEAN(AUXDATA(IDX))
      IF COUNT GT 2 THEN SENSOR_L1B_REF[16, IDX_STRUCT] = STDDEV(AUXDATA(IDX))




;=========== TO REMOVE AFTER VALIDATION ====================
;SENSOR_L1B_REF[5:16, IDX_STRUCT] = MISSING_VALUE_FLT
;===========================================================
...
;     ID = WHERE(ABS(SENSOR_L1B_REF[0,*]-DB_DATA_CT.DECIMAL_YEAR[RES[I]]) LE 0.000005 AND $ 
     ...
     IF ID[0] GT -1 THEN BEGIN
       IF N_ELEMENTS(CT_ID) EQ 0 THEN CT_ID = [ ID ] $
                                 ELSE CT_ID = [CT_ID,ID]
     ENDIF
  ENDFOR
...
; READ OZONE AND WATER VAPOUR FOR ALL EXTRACTIONS
  ...
  TMP   = WHERE(OZONE EQ MISSING_VALUE_FLT)
  IF TMP[0] NE -1 THEN OZONE[TMP]=REF_O3/1000.0
  OZONE = OZONE * 1000.0  
  
  WVAP  = REFORM(SENSOR_L1B_REF[15,*])
  TMP = WHERE(WVAP EQ MISSING_VALUE_FLT)
  IF TMP[0] NE -1 THEN WVAP[TMP]=REF_WV/10.0
  WVAP = WVAP * 10.0  
...
;  MANUAL_CS = []
  FOR I = 0l, N_ELEMENTS(RES)-1 DO BEGIN
     ID = WHERE(ABS(SENSOR_L1B_REF[0,*]-DB_DATA.DECIMAL_YEAR[RES[I]]) LE TOL AND SENSOR_L1B_REF[NUM_NON_REF+BRDF_BAND,*] NE MISSING_VALUE_FLT)
     IF ID[0] GT -1 THEN BEGIN
       ...
       IF (N_ELEMENTS(ID)) GT 1 THEN BEGIN
           ...
           ID=ID[ID_MIN]
       ENDIF
       IF N_ELEMENTS(GD_I) EQ 0 THEN BEGIN
         GD_I      = [I]
         GD_ID     = [ID]
         AUTO_CS   = [DB_DATA.AUTO_CS_1_MEAN[RES[I]]]
         MANUAL_CS = [DB_DATA.MANUAL_CS[RES[I]]]
       ENDIF ELSE BEGIN
         GD_I      = [GD_I, I]
         GD_ID     = [GD_ID,ID]
         AUTO_CS   = [AUTO_CS,  DB_DATA.AUTO_CS_1_MEAN[RES[I]]]
...
; REMOVE CSV BRDF DATABASE : THE INFORMATION IS STORED IN DIMITRI DATABASE
...
; SAVE UPDATED DATABASE 
  DB_DATA.BRDF_CS_MEAN[RES] = BRDF_CS 
  
  IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ' - SENDING DATA TO UPDATE DATABASE'
  STATUS = SAVE_DIMITRI_DATABASE(DB_DATA,VERBOSE=VERBOSE)

;----------------------
; SAVE RESULT IN NCDF FILE
        
  IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ' - UPDATE INGESTION NCDF FILES'
  FOR DBI = 0L,N_ELEMENTS(RES)-1 DO BEGIN
    NCDF_FILENAME =  INGESTION_FOLDER + DL + 'Site_' + REGION + DL + SENSOR + DL + 'Proc_' + PROC_VER + DL + STRTRIM(STRING(DB_DATA.YEAR[RES[DBI]]),2) + DL + DB_DATA.L1_INGESTED_FILENAME[RES[DBI]]
    
    ncid = NCDF_OPEN(NCDF_FILENAME, /WRITE)
    ;NCDF_ATTDEL, ncid, 'BRDF_CS_MEAN', /GLOBAL    ; optional if NCDF_ATTPUT function does not work properly without deletion before
    NCDF_ATTPUT, ncid , 'BRDF_CS_MEAN' , BRDF_CS[DBI] , /GLOBAL
    NCDF_CLOSE, ncid
  ENDFOR
