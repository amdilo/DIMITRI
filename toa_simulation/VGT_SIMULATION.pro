...
; INDEX IN ALL DATA VARIABLE    
  ...
  FOR GD=0l,N_ELEMENTS(GD_DATE)-1 DO BEGIN
    RES = WHERE(  ABS(VGT_DATA[0,*]-GD_DATE[GD]) LE TOL AND VGT_DATA[NUM_NON_REF,*] GE 0.0)
...
; ACCEPTABLE DATA
  ...
  SIMU_VGT  = MAKE_ARRAY(/FLOAT,NB_DATES,NUM_NON_REF+NB_BANDS_VGT,VALUE=0.0)
...
;   FIND ASSOCIATED BRDF BINS
    ...
    IF P1 GT BRDF_BIN_PERIOD/365.0 THEN BEGIN
      PRINT,'VGT_SIMULATION: ERROR, NO MERIS BINS NEAR VGT DATE'
      GOTO,NEXT_VGT_DATE
    ENDIF
...
; GET ASSOCIATED AATSR 1.6 MICRON KERNAL
    ...
    IF P1 GT BRDF_BIN_PERIOD/365.0 THEN BEGIN
      PRINT,'VGT_SIMULATION: ERROR, NO AATSR BINS NEAR VGT DATE'
      GOTO,NEXT_VGT_DATE
    ENDIF
    ...
    ATS_ROUJ[2] = ATS_K2[NUM_NON_ROU+ATS_1600_ID,ATS_TEMP_BINS[0]]

    IF ATS_ROUJ[0] EQ -999.0 THEN BEGIN       
      PRINT,'VGT_SIMULATION: ERROR, NO AATSR KERNALS AVAILABLE NEAR VGT DATE'
      GOTO,NEXT_VGT_DATE
    ENDIF  
...
; BRDF MODEL VALUES    
      ...
      IF  TOA_VZA LT MER_UC[2,MER_TEMP_BINS[0]] OR TOA_VZA GT MER_UC[3,MER_TEMP_BINS[0]] OR     $
        ...
        PRINT, 'ERROR, VGT GEOMETRY OUTSIDE OF BRDF MODEL OBSERVATIONS, GOING TO NEXT ACQUISITION'
...
; COMPUTE TRANSMISSION AT EACH BAND
    ...
    FOR TBAND=0,NB_BANDS_MERIS DO BEGIN
      TOZONE    = TOTAL(TO3*RSR_DATA[*,TBAND])/TOTAL(RSR_DATA[*,TBAND])
      TWVAP     = TOTAL(TWV*RSR_DATA[*,TBAND])/TOTAL(RSR_DATA[*,TBAND])  
      TGAS      = TOTAL(TGS*RSR_DATA[*,TBAND])/TOTAL(RSR_DATA[*,TBAND])
      TRANS[TBAND] = COMPUTE_TRANSMISSION(TOZONE,OZONE,TWVAP,WVAP,TGAS,TOA_SZA,TOA_VZA)
    ENDFOR
...
; CORRECT RHO FOR TRANSMISSION
    ...
    SSEN_RHO_COR = SSEN_RHO*(1.0-TRANS)
...
;   ADD HYPERSPECTRAL TRANSMISSION
    ...
    SSEN_RHO_HYP = SSEN_RHO_COR_HYP*(1.+TRANS_HYPER)
...
; GENERATE THE REFLECTANCE PLOT
    ...
    XYOUTS,0.88,0.9-0.035*0,'- '+LNAME[0],COLOR=LCOLOR[0],/normal
    XYOUTS,0.88,0.9-0.035*1,'- '+LNAME[1],COLOR=LCOLOR[1],/normal
    XYOUTS,0.88,0.9-0.035*2,'- '+LNAME[2],COLOR=LCOLOR[2],/normal
...
; OUTPUT CSV OF VGT RHO AND COMPUTED RHO
  ...
  FOR K=1,NUM_NON_REF-1 DO PRINTF,OUTF,FORMAT=FORMAT,VGT_SENSOR,VGT_PROC_VER,NON_REF_NAMES[K-1],GD_VGT_DATA[K,*]
