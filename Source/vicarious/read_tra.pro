...
;-----------------------------------------
 ...
 TAB = READ_ASCII(infile_down, DATA_START=8, HEADER=HEAD)
 ...
 DATA   = REFORM(TAB.field1, 7, N_ELEMENTS(wind), N_ELEMENTS(SZA), N_ELEMENTS(lambda))
 DATA   = TRANSPOSE(DATA,[1, 2, 0, 3])
 TRA_DOWN_LUT = CREATE_STRUCT('wind', wind, 'SZA', SZA, 'lambda', lambda, 'DATA', DATA)
...
;-----------------------------------------
 ...
 DATA = REFORM(TAB.field1, 7, N_ELEMENTS(VZA), N_ELEMENTS(lambda))
