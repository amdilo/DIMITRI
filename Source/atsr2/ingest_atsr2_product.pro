...
FUNCTION INGEST_ATSR2_PRODUCT,IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
  ...
  NB_FILES = N_ELEMENTS(IFILES)
	...
	;-----------------------------------------------
	; GET THE PROPER DATABASE STRUCTURE (SUNGLINT OR CLASSICAL DATABASE)
  ...
  DB_DATA.ESA_CHLOROPHYLL_MEAN = -1
	...
	;----------------------------------
	; DEFINE ATSR2 SPECIFIC PARAMETERS 
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ' - STARTING INGESTION LOOP ON ATSR2 PRODUCTS'
    ...
    CUR_FILENAME=TEMP[N_ELEMENTS(TEMP)-1]
    DB_DATA.L1_FILENAME[IN_FNAME]=CUR_FILENAME 
    ...
    NCDF_OUT.VAR_DTIME[IN_FNAME]  = DB_DATA.DECIMAL_YEAR[IN_FNAME]
		...
		FOR IN_DIR = 0,NB_DIRECTIONS-1 DO BEGIN
			...
			IFILE_TOA = GET_ATSR2_L1B_REFLECTANCE(IFILES[IN_FNAME],0,CUR_DIR,ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE)
			TOA_DIMS = SIZE(IFILE_TOA)

			IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ' - RETRIEVING GEOLOCATION INFORMATION'
			IFILE_GEO = GET_ATSR2_LAT_LON(IFILES[IN_FNAME],CUR_DIR,TOA_DIMS[2],TOA_DIMS[1],VERBOSE=VERBOSE)

			IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ' - RETRIEVING VIEWING GEOMETRIES'
			TEMP = GET_ATSR2_VIEWING_GEOMETRIES(IFILES[IN_FNAME],CUR_DIR,TOA_DIMS[2],TOA_DIMS[1],VERBOSE=VERBOSE)
        ...
        ; FIND SUNGLINT SPOT
          ...
          NB_PIX $
			...
			IF N_ELEMENTS(NO_QUICKLOOK) EQ 0 THEN BEGIN
				...
				IF FIX(CFIG_DATA.(1)[3]) EQ 1 THEN QL_STATUS =  GET_ATSR2_QUICKLOOK(IFILES[IN_FNAME],OUTPUT_QL_FILENAME,/ROI,/RGB,ICOORDS=ICOORDS,QL_QUALITY=QL_QUALITY,ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE,QL_DIR=CUR_DIR) $
					ELSE QL_STATUS =  GET_ATSR2_QUICKLOOK(IFILES[IN_FNAME],OUTPUT_QL_FILENAME,/ROI,ICOORDS=ICOORDS,QL_QUALITY=QL_QUALITY,ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE,QL_DIR=CUR_DIR)
			...
			FOR IN_BAND=0,NB_BANDS-1 DO BEGIN
				...
				TOA_REF=GET_ATSR2_L1B_REFLECTANCE(IFILES[IN_FNAME],IN_BAND,CUR_DIR,ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE)
      ...
      NCDF_OUT.VAR_WVAPSD[IN_FNAME]   = IFILE_AUX[11,2*IN_FNAME+IN_DIR] 
			...
			CS_METHOD_ARG = 'LCCA'
			CS_ATSR2_ARG = DIMITRI_CLOUD_SCREENING(SENSOR, SITE_TYPE, CS_RHO, 0, CS_METHOD_ARG, $
			...
			IF STRCMP(SITE_TYPE,'OCEAN',/FOLD_CASE) THEN BEGIN
				...
				CS_ATSR2_MAG = DIMITRI_CLOUD_SCREENING(SENSOR, SITE_TYPE, CS_RHO, CS_LATLON[*,2:5], CS_METHOD_MAG, $
			...
			ENDIF ELSE IF STRCMP(SITE_TYPE,'DESERT',/FOLD_CASE) THEN BEGIN
				...
				CS_ATSR2_MAG = DIMITRI_CLOUD_SCREENING(SENSOR, SITE_TYPE, CS_RHO, CS_LATLON[*,0:5], CS_METHOD_MAG, $
																							CS_CLASSIF_MATRIX=CS_CLASSIF_MATRIX_MAG, VERBOSE=VERBOSE, $
																							SITE_NAME=IREGION)     
			ENDIF ELSE BEGIN
					CS_METHOD_MAG = 'LCCA'
					CS_ATSR2_MAG = CS_ATSR2_ARG     
      ...
      DB_DATA.AUTO_CS_2_NAME[IN_FNAME] = AUTO_CS_2_NAME + ' - ' + CS_METHOD_MAG
			DB_DATA.AUTO_CS_1_MEAN[IN_FNAME] = MAX( [ DB_DATA.AUTO_CS_1_MEAN[IN_FNAME], CS_ATSR2_ARG ] )
			DB_DATA.AUTO_CS_2_MEAN[IN_FNAME] = MAX( [ DB_DATA.AUTO_CS_2_MEAN[IN_FNAME], CS_ATSR2_MAG ] )
