...
;* 
;* CALLING SEQUENCE:
;*      RES = GLINT_CALIBRATION(OUTPUT_FOLDER,SITE_NAME,SENSOR,PROC_VERSION, $
...
FUNCTION GLINT_CALIBRATION, SITE_NAME, SENSOR, PROC_VERSION,$
...
; RECORD THIS PROCESSING REQUEST IN A LOG
  ...
  TMP_DATE,SITE_NAME,SENSOR,PROC_VERSION,YEAR,CP_LIMIT,RP_LIMIT,WS_MAX,CONE_MAX,WAVREF,ABS_CALREF,STRTRIM(KEYWORD_SET(CLIM),2),CHL,TAUA_865_CLIM,AER,STRTRIM(KEYWORD_SET(PIX),2)
...
;	  WIND_SPEED_ALL[*]= REF_WIND
    ...
    FOR NUM_DIR=0, NB_DIRECTIONS-1 DO BEGIN
      ...
      IF DEBUG_MODE THEN PRINT, ' DEBUG_MODE - NB_PIX = ', NB_PIX
			...
			IF KEYWORD_SET(CLIM) THEN CHL = MAKE_ARRAY(NB_PIX,/DOUBLE,VALUE=CHL_CLIM[NCDF_MONTH-1]) $
													 ELSE CHL = MAKE_ARRAY(NB_PIX,/DOUBLE,VALUE=CHL_VALUE)
