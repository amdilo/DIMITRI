...
; ADD MODIS LAND/OCEAN EXCEPTION
  ...
  ED_SENS = STRARR(2)
  IF SENSOR1 EQ 'MODISA' THEN ED_SENS[0] = 'MODISA_O' ELSE ED_SENS[0] = SENSOR1 
  IF SENSOR2 EQ 'MODISA' THEN ED_SENS[1] = 'MODISA_O' ELSE ED_SENS[1] = SENSOR2   
...
; THRESHOLD VALUE
    ...
    IF AMC LT AMC_THRESHOLD THEN BEGIN
        ED_SENSOR1_SENSOR2[0:NUM_BANDS1+NUM_NON_REF-1,I_DAY]  = GD_SENSOR1_DATA[0:NUM_BANDS1+NUM_NON_REF-1,I_DAY]
        ED_SENSOR1_SENSOR2[NUM_BANDS1+NUM_NON_REF,I_DAY]      = AMC 
        TT = NUM_BANDS1+NUM_NON_REF+1 
        ED_SENSOR1_SENSOR2[TT:TT+NUM_BANDS1-1,I_DAY]          = GD_SENSOR1_DATA[TT-1:TT+NUM_BANDS1-2,I_DAY]
        
             
        ED_SENSOR2_SENSOR1[0:NUM_BANDS2+NUM_NON_REF-1,I_DAY]  = GD_SENSOR2_DATA[0:NUM_BANDS2+NUM_NON_REF-1,ID_MATCH_DAY[I_MATCH]]
        ED_SENSOR2_SENSOR1[NUM_BANDS2+NUM_NON_REF,I_DAY]      = AMC
        TT = NUM_BANDS2+NUM_NON_REF+1 
        ED_SENSOR2_SENSOR1[TT:TT+NUM_BANDS2-1,I_DAY]          = GD_SENSOR2_DATA[TT-1:TT+NUM_BANDS2-2,ID_MATCH_DAY[I_MATCH]]
        ED_NO_DATA = 1
    ENDIF
...
; SETUP WINDOW PROPERTIES
  ...
  FOR EDS=0,1 DO BEGIN
    ...
    FOR DBEI=0,2*TMP_BANDS+NUM_NON_REF-1 DO BEGIN
      ...
      if tband ne 'ERROR' then PRINTF,OUTF,FORMAT=FORMAT,ED_REGION,SENS[EDS],PVER[EDS],'TOA_REF_STD_'+TBAND,DATA[NUM_NON_REF+1+TMP_BANDS+SCOUNTER,*] 
        SCOUNTER++    
      ENDIF
    ENDFOR
