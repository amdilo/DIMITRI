...
; POPULATE ARRAY WITH INFORMATION
  ...
  FOR IN_FILE = 0l,NB_FILES-1 DO BEGIN
    ...
    TEMP_INF = WHERE(STRCMP(TEMP,'Input') EQ 1)
    TEMP_INF = TEMP_INF(N_ELEMENTS(TEMP_INF)-1)
    FILE_ARRAY[2,IN_FILE] = STRMID(TEMP[TEMP_INF+1],5,STRLEN(TEMP[TEMP_INF+1])) ;SITE
    FILE_ARRAY[3,IN_FILE] = TEMP[TEMP_INF+2] ;SENSOR
    FILE_ARRAY[4,IN_FILE] = STRMID(TEMP[TEMP_INF+3],5,STRLEN(TEMP[TEMP_INF+3])) ;PROC
