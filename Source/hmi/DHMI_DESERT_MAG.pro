...
;* 
;* PURPOSE:
;*      THIS PROGRAM MANAGE THE HMI (HUMAN/MAN INTERFACE) FOR THE DESERT VICARIOUS CALIBRATION METHOD
;*
;* CALLING SEQUENCE:
;*      DHMI_DESERT_MAG      
;*
...
; CALL INGEST INTERFACE
  ...
  CASE ACTION OF
    'PROCESS':  RES = DIMITRI_INTERFACE_DESERT(INGEST_REGION,INGEST_SENSOR,INGEST_PROC_VER,YEAR=IYEAR,VERBOSE=DHMI_DESERT_MAG_INFO.IVERBOSE)

    'ALL':      RES = DIMITRI_INTERFACE_DESERT(INGEST_REGION,INGEST_SENSOR,INGEST_PROC_VER,/ALL,YEAR=IYEAR,VERBOSE=DHMI_DESERT_MAG_INFO.IVERBOSE)
  ENDCASE
...
; CLOSE DISPLAY WIDGET AND REPORT STATUS
  ...
  CASE RES OF
    -1: MSG = ['One or more errors encountered','Please check database file and output plots']
     ...
     1: MSG = ['Successful']
  ENDCASE
  TMP = DIALOG_MESSAGE(['Desert Calibration Process : ','',MSG],/INFORMATION,/CENTER)


;--------------------------
; RETURN TO INGEST WIDGET 

  IF DHMI_DESERT_MAG_INFO.IVERBOSE EQ 1 THEN PRINT, 'DHMI_DESERT_MAG->PROCESS: RETURNING TO THE INGEST WIDGET'  
...
; GET LIST OF ALL SITE NAMES
 ...
 ; FILTER OUT SITE LIST WITH ONLY SITE TYPE = DESERT > DESERT METHOD IS ONLY VALID FOR DESERT TYPE
  ...
  IF SITE_SEARCH[0] EQ '' THEN BEGIN
    MSG='DHMI_DESERT_MAG: ERROR, NO DESERT SITES FOUND IN INPUT FOLDER, PLEASE INGEST L1B PRODUCTS FOR DESERT FIRST!'
...
IF KEYWORD_SET(VERBOSE) THEN PRINT,'DHMI_DESERT_MAG: SEARCHING FOR AUX DATA'
  ...
  IF AUX_DATA_CHANNELS_NB EQ 0 THEN BEGIN
    MSG='DHMI_DESERT_MAG: ERROR, NO AUXDAT FOUND IN FILE AUX_DATA !'
    TMP = DIALOG_MESSAGE(MSG,/ERROR,/CENTER)
    RETURN
  ENDIF
...
; BUILD UP THE WIDGET
  ...
  DHMI_DESERT_MAG_TLB     = WIDGET_BASE(COLUMN=1,TITLE=GET_DIMITRI_LOCATION('TOOL')+': DESERT CALIBRATION',XSIZE=XSIZE,XOFFSET=XLOC,YOFFSET=YLOC)
  ...
  DHMI_DESERT_MAG_TLB_PAR_LBL = WIDGET_LABEL(DHMI_DESERT_MAG_TLB_PAR,VALUE='CALIBRATION PARAMETERS : ')
