...
;* 
;* PURPOSE:
;*      COMPUTE RHOPATH/RHORAY RATIO FOR GIVEN TAUA AT GIVEN WIND, GEOMETRY AND BAND
;*      WITH MULTILINEAR INTERPOLATION IN 4-DIMENSIONNAL ARRAY 
;* 
;* CALLING SEQUENCE:
;*      RES = COMPUTE_XC(WIND, RAA, VZA, SZA, TAUA, IBAND)
;* 
;* INPUTS:
...
;**************************************************************************************
;**************************************************************************************
FUNCTION COMPUTE_XC, WIND, RAA, VZA, SZA, TAUA, IBAND, COS=COS, VERBOSE=VERBOSE
...
;-----------------------------------------
; CHECK XC_LUT IS LOADED

 IF N_ELEMENTS(XC_LUT) EQ 0 THEN BEGIN
   PRINT, FCT_NAME+":ERROR, XC_LUT NOT LOADED"
   RETURN, -1
 ENDIF
...
; APPLY POLYNOMIAL
 ...
 ZETA=MAKE_ARRAY(N0,/FLOAT,VALUE=0.)
 FOR I=0, N0-1 DO ZETA[I] = POLY(TAUA[I], REFORM(XC[*,I]))
