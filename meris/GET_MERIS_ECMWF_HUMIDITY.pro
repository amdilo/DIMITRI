...
;*
;* NAME:
;*      GET_MERIS_ECMWF_HUMIDITY       
;* 
;* PURPOSE:
;*      RETURNS THE ECMWF RELATIVE HUMIDITY OF A MERIS IMAGE
;* 
;* CALLING SEQUENCE:
;*      RES = GET_MERIS_ECMWF_HUMIDITY(FILENAME)      
;* 
;* INPUTS:
...
;*
;* OUTPUTS:
;*     ECMWF_HUMIDITY   - ECMWF RELATIVE HUMIDITY
;*
;* COMMON BLOCKS:
;*     NONE 
;*
...
FUNCTION GET_MERIS_ECMWF_HUMIDITY,FILENAME,ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE

;------------------------------------------------
; CHECK FILENAME IS NOMINAL

  IF FILENAME EQ '' THEN BEGIN
    PRINT, 'MERIS L1B ECMWF HUMIDITY: ERROR, INPUT FILENAME INCORRECT'
    RETURN,-1
  ENDIF
...
; IF ENDIAN SIZE NOT PROVIDED THEN GET VALUE
  ...
  IF N_ELEMENTS(ENDIAN_SIZE) EQ 0 THEN BEGIN
    IF KEYWORD_SET(VERBOSE) THEN BEGIN
      PRINT, 'MERIS L1B ECMWF HUMIDITY: NO ENDIAN SIZE PROVIDED, RETRIEVING...'
...
; OPEN THE FILE AND EXTRACT HEADER
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'MERIS L1B ECMWF HUMIDITY: OPENING PRODUCT'
...
; RETRIEVE: POSITION OF DSD, TP SUBSAMPLING FREQUENCY,LINE SIZE,TP PER LINE,DSD,OFFSET,DS_SIZE,NUMBER OF RECORDS, RECORD SIZE
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'MERIS L1B ECMWF HUMIDITY: RETRIEVING DSD INFORMATION'
...
; DEFINE ARRAY AND TEMPORARY TO HOLD DATA
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'MERIS L1B ECMWF HUMIDITY: DEFINING DATA ARRAYS FOR OUTPUT'
  ECMWF_HUMIDITY = UINTARR(NB_TP_LINE,TP_DSR_NUMBER)
...
; LOOP OVER EACH RECORD AND EXTRACT ALL ECMWFDATA  
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'MERIS L1B ECMWF HUMIDITY: STARTING LOOP FOR DATA EXTRACTION'
...
; SWAP ENDIAN IF NEEDED - MERIS DATA IS BIG ENDIAN
    ...
    IF ENDIAN_SIZE EQ 0 THEN ECMWF_HUMIDITY = SWAP_ENDIAN(ECMWF_HUMIDITY)
  
;---------------------------------------
; CLOSE THE FILE

  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'MERIS L1B ECMWF HUMIDITY: CLOSING PRODUCTS AND RELEASING THE LUN'
  CLOSE, PRD_VIG
  FREE_LUN, PRD_VIG
...
; FIND NUMBER OF PIXELS WITHIN MERIS RADIANCE FRAME
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'MERIS L1B ECMWF HUMIDITY: RETRIEVE DIMENSIONS OF RADIANCE PRODUCT'
...
; CONVERT VALUES INTO CORRECT UNITS
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'MERIS L1B ECMWF HUMIDITY: CONVERT VALUES INTO DEGREES'
  ECMWF_HUMIDITY   = FLOAT(ECMWF_HUMIDITY)*0.1
  
;---------------------------------------
; INTERPOLATE VALUES TO RADIANCE GRID

  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'MERIS L1B ECMWF HUMIDITY: REGRID DATA INTO RADIANCE PRODUCT DIMENSIONS'
  ECMWF_HUMIDITY   = CONGRID(ECMWF_HUMIDITY,LINE_SIZE,NB_RD_LINE,/MINUS_ONE,/INTERP)
  
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'MERIS L1B ECMWF HUMIDITY: RETURNING ECMWF HUMIDITY'
  RETURN,ECMWF_HUMIDITY
