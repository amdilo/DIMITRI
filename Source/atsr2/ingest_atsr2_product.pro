...
; COMPUTE TOA REFLECTANCE FOR EACH BAND 
       ...
       FOR IN_BAND=0,NB_BANDS-1 DO BEGIN
          ...
          L_TOA_REF=GET_ATSR2_L1B_REFLECTANCE(IFILES[IN_FNAME],IN_BAND,ATSR2_DIRC[IN_DIR],ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE)
			
;---------------------------------------
; CORRECT RELFECTANCE FOR SZA

          TOA_REF[IN_BAND,*] =  L_TOA_REF[ROI_INDEX]
          IF IN_BAND LE 3 THEN TOA_REF[IN_BAND,*] *= 0.01/COS(SZA*!DTOR)

;---------------------------------------
; COMPUTE MEAN AND STANDARD DEVIATION OVER ROI

          VALID = WHERE(TOA_REF[IN_BAND,*] GT 0.0,COUNT)
...
; APPLY CLOUD SCREENING
       ...
       CLOUD_MASK = DIMITRI_CLOUD_SCREENING(SENSOR,AT2_SITE_TYPE,CS_RHO,0,'LCCA',VERBOSE=VERBOSE)
...
; CALCULATE PERCENTAGE CLOUD
       ...
       CS_ATSR2     = FLOAT(NUM_CS)/FLOAT(NUM_PIX)

       IF CS_ATSR2 GT TEMP_AUTO_CS THEN TEMP_AUTO_CS = DOUBLE(CS_ATSR2)
