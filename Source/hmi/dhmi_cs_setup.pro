...
; THESE VALUES EXIST
  ...
  DB_IDX = WHERE(STRMATCH(DHMI_DB_DATA.SITE_NAME,CSITE)              AND $
...
; SITES, SENSORS AND PROCESSING VERSIONS
  ...
  ASITES = DHMI_DB_DATA.SITE_NAME[UNIQ(DHMI_DB_DATA.SITE_NAME,SORT(DHMI_DB_DATA.SITE_NAME))];,'ALL']
...
; FSC_FIELDS AND DROPLIST
  ...
  DHMI_CS_SETUP_TLB = WIDGET_BASE(COLUMN=1,TITLE=GET_DIMITRI_LOCATION('TOOL')+': CLOUD SCREENING SETUP',XSIZE=XSIZE,$
