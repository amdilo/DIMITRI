...
; UPDATE THE FIELD VALUES
  ...
  RES = WHERE(DHMI_DB_DATA.SITE_NAME EQ CURRENT_SITE AND DHMI_DB_DATA.SENSOR EQ DHMI_P2_INFO.VGT_SENSOR,COUNT)
  IF COUNT GT 0 THEN BEGIN
...
; GET AVAILABLE VEGETATION PROCESSING VERSIONS THROUGH DATABASE 
  ...
  RES = WHERE(DHMI_DB_DATA.SITE_NAME EQ CURRENT_SITE AND DHMI_DB_DATA.SENSOR EQ VGT_SENSOR,COUNT)
  IF COUNT GT 0 THEN BEGIN
    TEMP = DHMI_DB_DATA.PROCESSING_VERSION[RES]
    TEMP = TEMP[UNIQ(TEMP,SORT(TEMP))]
...
; DEFINE THE MAIN WIDGET 
  ...
  SML_BTN = 80
  SML_FSC_X=7
  DHMI_P2_TLB = WIDGET_BASE(COLUMN=1,TITLE=GET_DIMITRI_LOCATION('TOOL')+': VGT SIM SETUP',XSIZE=XSIZE,$
