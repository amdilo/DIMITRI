...
FUNCTION INGEST_PARASOL_PRODUCT,IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
  ...
  NB_DIRECTIONS = SENSOR_DIRECTION_INFO(SENSOR)
    ...
    DATE_SEC = STRMID(TDATE,12,2)
		...
		IFILE_DATE[3,NB_DIRECTIONS*IN_FNAME:NB_DIRECTIONS*IN_FNAME+NB_DIRECTIONS-1] = JULDAY(FIX(ACQ_MONTH),FIX(ACQ_DAY),FIX(ACQ_YEAR)) - JULDAY(1,0,FIX(ACQ_YEAR))
    IFILE_DATE[4,NB_DIRECTIONS*IN_FNAME:NB_DIRECTIONS*IN_FNAME+NB_DIRECTIONS-1] = DATETIME_TO_DECIMAL_YEAR(FIX(ACQ_YEAR),FIX(ACQ_MONTH),FIX(ACQ_DAY),FIX(DATE_HR),FIX(DATE_MIN),FIX(DATE_SEC),VERBOSE=VERBOSE)
