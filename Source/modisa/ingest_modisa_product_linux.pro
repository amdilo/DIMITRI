...
; KEYWORD PARAMETER CHECK - NOTE, ASSUMES ALL PRODUCT ARE RELATED TO THE SAME REGION/PROCESSING
  ...
  IF STRUPCASE(SITE_TYPE) EQ 'OCEAN' THEN AQA_SENSOR = SENSOR+'_O' $
  																	 ELSE AQA_SENSOR = SENSOR+'_L'
...
; START MAIN LOOP OVER EACH IFILE
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'MODISA L1B INGEST: STARTING INGESTION LOOP ON MERIS PRODUCTS'
    ...
    DB_DATA.L1_FILENAME[IN_FNAME] = TEMP[N_ELEMENTS(TEMP)-1]
  	...
  	IF KEYWORD_SET(VERBOSE) THEN BEGIN
      PRINT, 'MODISA INGEST [' + STRTRIM(STRING(IN_FNAME+1),1) + ' / ' + STRTRIM(STRING(NB_FILES),1) + '] : ' + DB_DATA.L1_FILENAME[IN_FNAME]
  	ENDIF
...
; RETRIEVE INPUT FILE GEOLOCATION
    ...
    NCDF_INGEST_STRUCT.GLOBAL_ATT.L1_FILENAME = DB_DATA.L1_FILENAME[IN_FNAME] 
		...
		IF N_ELEMENTS(NO_QUICKLOOK) EQ 0 THEN BEGIN
			...
			IF FIX(CFIG_DATA.(1)[3]) EQ 1 THEN QL_STATUS =  GET_MODISA_QUICKLOOK_LINUX(IFILES[IN_FNAME],OUTPUT_QL_FILENAME,/ROI,/RGB,ICOORDS=ICOORDS,VERBOSE=VERBOSE) $
         ELSE QL_STATUS =  GET_MODISA_QUICKLOOK_LINUX(IFILES[IN_FNAME],OUTPUT_QL_FILENAME,/ROI,ICOORDS=ICOORDS,QL_QUALITY=QL_QUALITY,VERBOSE=VERBOSE)
