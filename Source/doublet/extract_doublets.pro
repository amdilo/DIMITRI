...
;*        11 MAY 2015 - NCG / MAGELLIUM - UPDATE INPUT FORMAT : CATCH SELECTED INGESTED PRODUCT IN NetCDF FORMAT 
;*                                AND CONVERT THEM IN TABLE WITH SAME FORMAT AS OLD TOA_REF.dat FILES (DIMITRI V4)
;*
;* VALIDATION HISTORY:
...
;*        02 JUN 2015 - NCG / MAGELLIUM - WINDOWS 32-BIT MACHINE IDL 8.0: COMPILATION AND OPERATION 
;*                                  NOMINAL, TESTED FOR AATSR VS MODIS
;*
;**************************************************************************************
;**************************************************************************************
...
; CHECKS OFFSET AND PERCENTAGES ARE OK
  ...
  IF  CP_LIMIT  GT 1.0 OR CP_LIMIT  LT 0.0 OR $
    ...
    PRINT, FCT_NAME + ': ERROR, CLOUD/ROI PERCENTAGES OUT OF RANGE'
    RETURN, STATUS_ERROR
  ENDIF
  
  IF KEYWORD_SET(VERBOSE) THEN BEGIN
    PRINT,FCT_NAME + ': REGION = ',ED_REGION,' SENSOR1 = ',SENSOR1,' PROC_VER1 = ',PROC_VER1
    PRINT,FCT_NAME + ': REGION = ',ED_REGION,' SENSOR2 = ',SENSOR2,' PROC_VER2 = ',PROC_VER2
    PRINT,FCT_NAME + ': DAY_OFFSET = ',DAY_OFFSET,' CLOUD % = ',CP_LIMIT,' ROI % = ',RP_LIMIT
  ENDIF
...
; CHECK INPUT SENSORS AND PROC_VERSIONS 
  ...
  IF STRCMP(SENSOR1,SENSOR2) EQ 1 AND $
    ...
    PRINT, FCT_NAME + ': ERROR, TRYING TO COMPARE THE SAME DATA'
...
;-----------------------------------------
; DEFINE SENSOR, PROC_VERSION AND NUMBER FO BANDS ARRAYS

  SENSOR1_BAND_INFOS = GET_SENSOR_BAND_INFO(SENSOR1)
  SENSOR2_BAND_INFOS = GET_SENSOR_BAND_INFO(SENSOR2)
  NB_DIRECTIONS = SENSOR_DIRECTION_INFO(SENSOR1)
  NB_DIRECTIONS1 = NB_DIRECTIONS[0]
  NB_DIRECTIONS = SENSOR_DIRECTION_INFO(SENSOR2)
  NB_DIRECTIONS2 = NB_DIRECTIONS[0]

  SENS = [SENSOR1,SENSOR2]
  PVER = [PROC_VER1,PROC_VER2]
  BANDS = [SENSOR1_BAND_INFOS.NB_BAND,SENSOR2_BAND_INFOS.NB_BAND]
  DIRS  = [NB_DIRECTIONS1, NB_DIRECTIONS2]
...
;----------------------------------------
; SET TEMPORARY SENSOR TOA DATA FILENAME

  S1_ADATA_FILE = STRING(OFOLDER+'TOA_REF_'+ED_REGION+'_'+SENSOR1+'_'+PROC_VER1+'.dat')
  S2_ADATA_FILE = STRING(OFOLDER+'TOA_REF_'+ED_REGION+'_'+SENSOR2+'_'+PROC_VER2+'.dat')
  SENSOR_L1B_REF_FILE  = [S1_ADATA_FILE, S2_ADATA_FILE]

;----------------------------------------
; LOOP OVER BOTH SENSORS TO EXTRACT DATA 
; WITHIN CLOUD/ROI PARAMETERS

  IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': STARTING LOOP OVER BOTH SENSORS DATA'
...
;    IF EDS EQ 1 THEN SENSOR_ALL_DATA = S2_ADATA
    ...
    RES = WHERE(STRCMP(DB_DATA.SITE_NAME,ED_REGION)             EQ 1 AND $
                ...
                DB_DATA.ROI_PIX_NUM GE PX_THRESH[EDS])            

    IF RES[0] EQ -1 THEN BEGIN
      PRINT, FCT_NAME + ': ERROR, NO SENSOR DATA FOUND WITHIN PIXEL THRESHOLD'  
      RETURN, STATUS_NODATA
    ENDIF
...
; WITHIN THE CLOUD PERCENTAGE 
    ...
    IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': STARTING LOOP OVER GOOD DATES FOR CLOUD PERCENTAGE'  
    FOR I_CS=0l,N_ELEMENTS(RES)-1 DO BEGIN
      ...
      IF (DB_DATA.MANUAL_CS[RES[I_CS]] EQ 0.0) OR $
        (DB_DATA.AUTO_CS_1_MEAN[RES[I_CS]] LE CP_LIMIT AND DB_DATA.AUTO_CS_1_MEAN[RES[I_CS]] GT -1.0) THEN BEGIN
        GD_YEAR = [GD_YEAR,DB_DATA.YEAR[RES[I_CS]]]
        GD_DATE = [GD_DATE,DB_DATA.DECIMAL_YEAR[RES[I_CS]]]
        GD_PIX = [GD_PIX,DB_DATA.ROI_PIX_NUM[RES[I_CS]]]
        GD_MAN = [GD_MAN,DB_DATA.MANUAL_CS[RES[I_CS]]]
        GD_AUT = [GD_AUT,DB_DATA.AUTO_CS_1_MEAN[RES[I_CS]]]
        GD_NAME= [GD_NAME,DB_DATA.L1_FILENAME[RES[I_CS]]]
        GD_NCDF_NAME= [GD_NCDF_NAME,DB_DATA.L1_INGESTED_FILENAME[RES[I_CS]]]
        NB_FILES = NB_FILES+1
        CONTINUE
      ENDIF  
    ...
    IF N_ELEMENTS(GD_DATE) EQ 1 THEN BEGIN
       PRINT, FCT_NAME + ': ERROR, NO SENSOR DATA FOUND WITHIN CLOUD THRESHOLD'
       RETURN, STATUS_NODATA
    ENDIF

    GD_YEAR = GD_YEAR[1:N_ELEMENTS(GD_YEAR)-1]
    ...
    GD_NCDF_NAME = GD_NCDF_NAME[1:N_ELEMENTS(GD_NCDF_NAME)-1]
    
  ;---------------------------------------
  ; COMPUTE SENSOR_L1B_REF TABLE FROM NCDF FILES
  ;---------------------------------------

  ;----------------------
  ; GET INGESTION OUTPUT FILENAME FROM DATABASE
  
  SEARCH_FOLDER = INPUT_FOLDER + DL + 'Site_' + ED_REGION + DL + SENS[EDS] + DL + 'Proc_' + PVER[EDS] + DL
  I_FILES = SEARCH_FOLDER + DL + STRTRIM(STRING(GD_YEAR),2) + DL + GD_NCDF_NAME
  ...
  NB_BANDS = BANDS[EDS]
  NB_DIRECTIONS  = DIRS[EDS]
  SENSOR_ALL_DATA = DBLARR(NUM_NON_REF+2*NB_BANDS, NB_FILES*NB_DIRECTIONS)

  ;----------------------
  ; LOOP OVER NCDF FILE IN PROCESSING DATASET TO FILL DATASET TABLE
  ...
  FOR IDX_FILE=0, NB_FILES-1 DO BEGIN
    ...
    ; GET DECIMAL YEAR FROM DATABASE
    IDX_DATE = WHERE(STRCMP(GD_NCDF_NAME,CUR_NCFILE) EQ 1, COUNT)
    IF COUNT EQ 0 THEN BEGIN
      IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': ERROR - FILE ',CUR_NCFILE,' NOT FOUND IN DATABASE EXTRACT'
      RETURN, STATUS_ERROR
    ENDIF
    
    DECIMAL_YEAR = GD_DATE(IDX_DATE[0])
    ...
    FOR NUM_DIR=0, NB_DIRECTIONS-1 DO BEGIN
      ...
      SENSOR_ALL_DATA[0, IDX_STRUCT] = DECIMAL_YEAR
      SENSOR_ALL_DATA[1, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.VZA[IDX_CLEAR,NUM_DIR])
      SENSOR_ALL_DATA[2, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.VAA[IDX_CLEAR,NUM_DIR])
      SENSOR_ALL_DATA[3, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.SZA[IDX_CLEAR,NUM_DIR])
      SENSOR_ALL_DATA[4, IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.SAA[IDX_CLEAR,NUM_DIR])

      ; LOOP ON VALID BANDS
      FOR BAND=0, NB_BANDS-1 DO BEGIN
        RTOA = NCDF_INGEST_STRUCT.VARIABLES.REFL_BAND[IDX_CLEAR, NUM_DIR, BAND]
        IDX = WHERE(RTOA NE MISSING_VALUE_FLT, COUNT)
        IF COUNT NE 0 THEN SENSOR_ALL_DATA[NUM_NON_REF+BAND, IDX_STRUCT] = MEAN(RTOA(IDX))
        IF COUNT GT 2 THEN SENSOR_ALL_DATA[NUM_NON_REF+NB_BANDS+BAND, IDX_STRUCT] = STDDEV(RTOA(IDX))
      ENDFOR
      
      AUXDATA = NCDF_INGEST_STRUCT.VARIABLES.ERA_OZONE[IDX_CLEAR,NUM_DIR]
      IDX = WHERE(AUXDATA NE MISSING_VALUE_FLT, COUNT)
      IF COUNT NE 0 THEN SENSOR_ALL_DATA[5, IDX_STRUCT] = MEAN(AUXDATA(IDX))
      IF COUNT GT 2 THEN SENSOR_ALL_DATA[6, IDX_STRUCT] = STDDEV(AUXDATA(IDX))
      
      AUXDATA = NCDF_INGEST_STRUCT.VARIABLES.ERA_PRESSURE[IDX_CLEAR,NUM_DIR]
      IDX = WHERE(AUXDATA NE MISSING_VALUE_FLT, COUNT)
      IF COUNT NE 0 THEN SENSOR_ALL_DATA[7, IDX_STRUCT] = MEAN(AUXDATA(IDX))
      IF COUNT GT 2 THEN SENSOR_ALL_DATA[8, IDX_STRUCT] = STDDEV(AUXDATA(IDX))
      
      ; NO INFORMATION AVAILABLE FOR AUXDATA VARIABLES: VAR_RHUMMU, VAR_RHUMSD, VAR_ZONALMU, VAR_ZONALSD, VAR_MERIDMU, VAR_MERIDSD
      ; SET TO -999.0
      SENSOR_ALL_DATA[9:14, IDX_STRUCT] = MISSING_VALUE_FLT
      
      AUXDATA = NCDF_INGEST_STRUCT.VARIABLES.ERA_WATERVAPOUR[IDX_CLEAR,NUM_DIR]
      IDX = WHERE(AUXDATA NE MISSING_VALUE_FLT, COUNT)
      IF COUNT NE 0 THEN SENSOR_ALL_DATA[15, IDX_STRUCT] = MEAN(AUXDATA(IDX))
      IF COUNT GT 2 THEN SENSOR_ALL_DATA[16, IDX_STRUCT] = STDDEV(AUXDATA(IDX))
...
;---------------------------------------
; END - COMPUTE SENSOR_L1B_REF STRUCTURE FROM NCDF FILES

;---------------------------------------
; SAVE SIMILAR REF_TOA.dat TABLE TO TEMPORARY FILE FOR ANGULAR MATCHING PROCESS
  SAVE, SENSOR_ALL_DATA, FILENAME=SENSOR_L1B_REF_FILE[EDS]
...
; ACCEPTABLE DATA
    ...
    ENDIF ELSE BEGIN
      PRINT, FCT_NAME + ': ERROR DURING DOUBLET EXTRACTION, NO GOOD DATES FOUND FOR SENSOR ',SENS[EDS]
      RETURN, STATUS_NODATA
    ENDELSE
  ENDFOR ;END OF LOOP ON SENSOR1 AND SENSOR2 FILTERING
  IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': COMPLETED LOOP ON GOOD DATES FOR CLOUD TESTING'
...
; OVER WRITE THE SENSOR DATA
  ...
  IF N_ELEMENTS(IDX) LT 2 THEN BEGIN
    PRINT,FCT_NAME + ':ERROR NO MATCHING SADE DATA FOR ANGLE CRITERA'
    RETURN,-3
  ENDIF
...
; AND FIND MATCHES
  ...
  ED_NO_DATA = 0
  IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': STARTING LOOP OVER EACH GOOD DAY IN SENSOR1'
  FOR I_DAY=0L,SIZE_S1[2]-1 DO BEGIN
  IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': COMPUTING FOR DAY ITERATION = ',I_DAY
...
; THRESHOLD VALUE, ED=Extracted Doublet
  ...
  IF ED_NO_DATA EQ 0 THEN BEGIN
    PRINT,FCT_NAME + ': ERROR, NO VALUES BELOW AMC THRESHOLD RETRIEVED'
    RETURN, STATUS_ERROR
  ENDIF
...
; SAVE ONLY DATA WHERE MATCHES WERE FOUND
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': SAVING AND RETURNING ONLY THE GOOD DATA'
...
; CREATE AN OUTPUT CSV FILE CONTAINING ALL DATA 
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': SAVING DATA AS A CSV FILE'
  ...
  TMP_AUX = ['NUM_ROI_PIX','AUTO_CS_1_MEAN','MANUAL_CS']
...
; SETUP WINDOW PROPERTIES
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': STARTING LOOP TO OUTPUT SENSOR DATA TO CSV'  
  FOR EDS=0,1 DO BEGIN
    IF EDS EQ 0 THEN BEGIN
