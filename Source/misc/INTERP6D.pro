...
;*
;* PURPOSE:
;*      FUNCTION TO PERFORM 6D INTERPOLATION THROUGH A 6-D LOOK-UP TABLE (DIRECT DATA RANGE RESTRICTION METHOD)
;*
;* CALLING SEQUENCE:
...
FUNCTION INTERP6D, DIM1_LUT, DIM2_LUT, DIM3_LUT, DIM4_LUT, DIM5_LUT, DIM6_LUT, OUT_LUT, $
                    DIM1, DIM2, DIM3, DIM4, DIM5, DIM6, DIM1_BKPT_VALUES, DIM2_BKPT_VALUES, $
                    DIM3_BKPT_VALUES, DIM4_BKPT_VALUES, DIM5_BKPT_VALUES, DIM6_BKPT_VALUES, INVERSE_Z=INVERSE_Z
  ...
  FOR INC=0,NB_VAL-1 DO BEGIN
    ...
    IF DIM1_DOINTERP(INC) EQ 0 THEN BEGIN
      ...
      IF KEYWORD_SET(INVERSE_Z) THEN BEGIN
          ; IN CASE OF LUT INVERSION LAST DIMENSION (DIM6) IS THE TARGET VALUE TO FIND
          DIM1_INV = DIM6_LUT(INDX_DIM1_LUT_BOL)
      ENDIF ELSE BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      IF KEYWORD_SET(INVERSE_Z) THEN BEGIN
        ; FIND PROPER DIM6 VALUE REGARDING CURRENT DIM1 VALUE BW. DIM1 RANGE (BOL/TOL)
        DIM1_INV= INTERP1D_MULTIY([CUR_DIM1_VAL_BOL, CUR_DIM1_VAL_TOL], [ [DIM6_LUT(INDX_DIM1_LUT_BOL)] , [DIM6_LUT(INDX_DIM1_LUT_TOL)] ], CUR_DIM1_VAL);
      ENDIF ELSE BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      DIM6_LUT_RNG_DIM4_TOL = DIM6_LUT_RNG_DIM3_TOL(INDX_DIM4_LUT_TOL)
    ...
    INDX_DIM5_LUT_BOL = WHERE(DIM5_LUT_RNG_DIM4_BOL EQ CUR_DIM5_VAL_BOL)
    ...
    ; FOCUS ON REMAINING DATA ON 6TH DIMENSION LIMITED TO 1D 
    ; FILTER OUT NAN VALUES ON X
    CUR_DIM6_VAL=DIM6(INC)
    INDX_XNAN=WHERE(FINITE(DIM5_INV, /NAN),COUNT_NAN,COMPLEMENT=INDX_XVLD)
    IF COUNT_NAN EQ N_ELEMENTS(DIM5_INV) THEN BEGIN
      OUT_VAL(INC)=!VALUES.F_NAN
      CONTINUE ; NEXT INC VALUE IN FOR LOOP
    ENDIF
    DIM5_INV=DIM5_INV(INDX_XVLD);
    
    IF KEYWORD_SET(INVERSE_Z) THEN BEGIN
      DIMOUT_LUT=OUT_LUT_RNG_DIM5_BOL(INDX_XVLD);
      ; LOOK FOR INVERTED VALUE OF DIM5 THROUGH DIMOUT VALUE
      OUT_VAL(INC)=INTERP1D(DIM5_INV,DIMOUT_LUT,CUR_DIM6_VAL);
    ENDIF ELSE BEGIN
      ; LOOK FOR DIMOUT THROUGH DIM6 DATA RANGE @ CUR_DIM5_VAL
      OUT_VAL(INC)=INTERP1D(DIM6_LUT_RNG_DIM5_BOL,DIM5_INV,CUR_DIM6_VAL);
    ENDELSE
