...
;*        15 JAN 2015 - B ALHAMMOUD  - ADDED PROCESS 5 (DESERT CALIBRATION)
;*        21 JAN 2015 - B ALHAMMOUD  - CHANGED DIMITRI V3.1.0 TO DIMITRI V3.1.1 PREVIOUSLY CALLED V3.1a
...
; CALL FUNCTIONALITY DEPENDING ON ACTION REQUEST
  ...
  CASE ACTION OF
    ...
    'SSV_CS'          : DHMI_SSV_CS,              GROUP_LEADER=EVENT.TOP,VERBOSE=DHMI_INFO.IVERBOSE
    'BRDF_CS'         : DHMI_BRDF_CS,             GROUP_LEADER=EVENT.TOP,VERBOSE=DHMI_INFO.IVERBOSE
    ...
    'PROCESS 3'       : DHMI_PROCESS_3,           GROUP_LEADER=EVENT.TOP,VERBOSE=DHMI_INFO.IVERBOSE
    'PROCESS 4'       : DHMI_PROCESS_4,           GROUP_LEADER=EVENT.TOP,VERBOSE=DHMI_INFO.IVERBOSE
    'PROCESS 5'       : DHMI_PROCESS_5,           GROUP_LEADER=EVENT.TOP,VERBOSE=DHMI_INFO.IVERBOSE
    ...
    'HELP'            : BEGIN
                        ...
                        END
    'ABOUT'           : BEGIN
                          MSG   = [	'DIMITRI Version '+DIMITRI_VERSION,'Release Date: 30/01/2015'              ,$
                                    'IDL 8.2.3 Or Higher required','___________________',''             ,$
                                    'Email: ','=> Dimitri@argans.co.uk'                  ,$
                                    'Authors:','=> Marc Bouvet (ESA-ESTEC)','=> Bahjat Alhammoud (ARGANS Ltd)',$
                                        '=> Kathryn Barker (ARGANS Ltd)','=> Manuel Arias Ballesteros (ARGANS Ltd)']
                          ABOUT = DIALOG_MESSAGE(MSG,/INFORMATION,TITLE = 'DIMITRI V'+DIMITRI_VERSION+': ABOUT',/CENTER)
                       END
 ENDCASE
...
; LOAD PNG IMAGES
  ...
  CLOUD     = GET_DIMITRI_LOCATION('CLOUD_PNG')      
...
; DEFINE MAIN WIDGET BASE
  ...
  DTLB = WIDGET_BASE(XSIZE=XSIZE,YSIZE=YSIZE,XOFFSET=XLOC,YOFFSET=YLOC,TITLE='DIMITRI V'+DIMITRI_VERSION,COLUMN=1)

;---------------------------
; ADD DRAW_WIDGET FOR MAIN TITLE IMAGE

  DTLB_IMG = WIDGET_DRAW(DTLB,XSIZE=DIMS[2],YSIZE=DIMS[3]-3,SENSITIVE=0,RETAIN=2,/ALIGN_CENTER)
...
;---------------------------
; ADD CLOUD IMAGE AND BUTTONS

  IF KEYWORD_SET(VERBOSE) THEN PRINT,'DIMITRI_HMI: ADDING CLOUD OPTION BOX'
  DTLB_TOP_CLOUD       = WIDGET_BASE(DTLB_TOP,COLUMN=1,FRAME=1)
  DTLB_TOP_CLOUD_B     = WIDGET_BASE(DTLB_TOP_CLOUD)
  DTLB_TOP_CLOUD_IMG   = WIDGET_DRAW(DTLB_TOP_CLOUD_B,XSIZE=146,YSIZE=23,SENSITIVE=0,RETAIN=2)
  DTLB_TOP_CLOUD_C     = WIDGET_BASE(DTLB_TOP_CLOUD,COLUMN=1,/ALIGN_CENTER)
  DTLB_TOP_CLOUD_BTN   = WIDGET_BUTTON(DTLB_TOP_CLOUD_C,VALUE='Manual Screening',UVALUE='MANUAL_CS',XSIZE=MAIN_BTN_SIZE,EVENT_PRO=BTEVENT)
  DTLB_TOP_CLOUD_BTN   = WIDGET_BUTTON(DTLB_TOP_CLOUD_C,VALUE='SSV Analysis', UVALUE='SSV_CS', XSIZE=MAIN_BTN_SIZE,EVENT_PRO=BTEVENT)
  DTLB_TOP_CLOUD_BTN   = WIDGET_BUTTON(DTLB_TOP_CLOUD_C,VALUE='BRDF Analysis',UVALUE='BRDF_CS',XSIZE=MAIN_BTN_SIZE,EVENT_PRO=BTEVENT)
...
; ADD PROCESS IMAGE AND BUTTONS
  ...
  DTLB_TOP_PROC_BTN     = WIDGET_BUTTON(DTLB_TOP_PROC_C,VALUE='Rayleigh Cal.',UVALUE='PROCESS 3',XSIZE=MAIN_BTN_SIZE,EVENT_PRO=BTEVENT)
  DTLB_TOP_PROC_BTN     = WIDGET_BUTTON(DTLB_TOP_PROC_C,VALUE='Glint Cal.',UVALUE='PROCESS 4',XSIZE=MAIN_BTN_SIZE,EVENT_PRO=BTEVENT)
  DTLB_TOP_PROC_BTN     = WIDGET_BUTTON(DTLB_TOP_PROC_C,VALUE='Desert Cal.',UVALUE='PROCESS 5',XSIZE=MAIN_BTN_SIZE,EVENT_PRO=BTEVENT)
...
; REALISE THE WIDGET AND DISPLAY THE PNG IMAGES
  ...
  WIDGET_CONTROL, DTLB_TOP_CLOUD_IMG, GET_VALUE=D3
  WSET, D3
  TV, CLOUD_IMAGE, TRUE=1

  WIDGET_CONTROL, DTLB_TOP_PROCESS_IMG, GET_VALUE=D4
  WSET, D4
  TV, PROCESS_IMAGE, TRUE=1
  
  WIDGET_CONTROL, DTLB_TOP_VISUALISE_IMG, GET_VALUE=D5
  WSET, D5
