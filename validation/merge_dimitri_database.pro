...
; DEFINE BOTH DATABASE FILES
  ...
  DB1_FILE = ''
  DB2_FILE = GET_DIMITRI_LOCATION('DATABASE')
...
; READ IN BOTH DATABASES
  ...
  DB2_DATA     = READ_ASCII(DB2_FILE,TEMPLATE=DB_TEMPLATE)
  COUNTER      = 0

; FIND INDEX OF ALL CHANGES TO MANUAL CS IN DB1

  CHID = WHERE(DB1_DATA.MANUAL_CS NE -1, COUNT)
...
; LOOP OVER EACH INDEX VALUE
  ...
  FOR II=0,COUNT-1 DO BEGIN
    ...
    MATCH = WHERE(DB2_DATA.FILENAME EQ DB2_DATA.FILENAME[CHID[II]]  AND $
                  DB2_DATA.REGION EQ DB2_DATA.REGION[CHID[II]]      AND $
                  DB2_DATA.SENSOR EQ DB2_DATA.SENSOR[CHID[II]]      AND $
                  DB2_DATA.PROCESSING_VERSION EQ DB2_DATA.PROCESSING_VERSION[CHID[II]],MCOUNT)
