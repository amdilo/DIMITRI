...
;  SBI_FILE = STRING(MAIN_DIRC+'Bin'+DL+'DIMITRI_Band_Centre_Index.txt')
  SBI_FILE = GET_DIMITRI_LOCATION('BAND_INDEX')

  RES = FILE_INFO(SBI_FILE)
  IF RES.EXISTS EQ 0 THEN BEGIN
    PRINT, 'SENSOR BAND INDEX: ERROR, SENSOR INFORMATION FILE NOT FOUND'
    RETURN,-1
  ENDIF
...
; CHECK INPUT INDEX
  ...
  IF FIX(BINDEX) EQ 0 THEN BEGIN 
    PRINT, 'SENSOR BAND INDEX: ERROR, BAND INDEX OUT OF RANGE'
    RETURN,-1
  ENDIF
...
;FIND INDEX OF INPUT INDEX
  ...
  IF RES[0] EQ -1 OR N_ELEMENTS(RES) GT 1 THEN BEGIN
    PRINT, 'SENSOR BAND INDEX: ERROR, SENSOR INDEX RETRIEVAL'
    RETURN,-1  
  ENDIF
