...
;*      TO LAUNCH THE ARGANS RAYLEIGH SENSOR TO SIMULATION SIMULATION 
;*
;* CALLING SEQUENCE:
;*      DHMI_RAYLEIGH_ARG      
;*
...
; RAYLEIGH CAL
  ...
  IF DHMI_P3_INFO.IVERBOSE EQ 1 THEN PRINT,'DHMI_RAYLEIGH_ARG->START: RUNNING RAYLEIGH SIMULATION'  

  RES = DIMITRI_INTERFACE_RAYLEIGH_ARG(P3_REGION,P3_SENSOR,P3_PROC,P3_YEAR,P3_CSPERCENT,P3_ROIPERCENT,$
...
; CLOSE DISPLAY WIDGET AND REPORT STATUS
  ...
  P3_ERR:
  IF DHMI_P3_INFO.IVERBOSE EQ 1 THEN PRINT, 'DHMI_RAYLEIGH_ARG->START: DESTROYING THE POP DISPLAY'
  IF N_ELEMENTS(INFO_WD) GT 0 THEN WIDGET_CONTROL,INFO_WD,/DESTROY
  WAIT,0.5

  IF N_ELEMENTS(RES) NE 0 THEN BEGIN
    IF DHMI_P3_INFO.IVERBOSE EQ 1 THEN PRINT, 'DHMI_RAYLEIGH_ARG->START: USE OUTPUT TO IDENTIFY CLOSING MESSAGE'
    CASE RES OF
      -1: MSG = ['One or more errors encountered','Please check log, database file and output plots']
       0: MSG = ['No Rayleigh products found']
       1: MSG = ['Successful']
    ENDCASE
    TMP = DIALOG_MESSAGE(['Rayleigh ARG Simulation Process : ','',MSG],/INFORMATION,/CENTER)
  ENDIF

;--------------------------
; RETURN TO SUNGLINT WIDGET 

  IF DHMI_P3_INFO.IVERBOSE EQ 1 THEN PRINT, 'DHMI_RAYLEIGH_ARG->START: RETURNING TO THE RAYLEIGH WIDGET'  
...
; DESTROY THE WIDGET
  ...
  IF DHMI_P3_INFO.IVERBOSE EQ 1 THEN PRINT,'DHMI_RAYLEIGH_ARG->EXIT: DESTROYING RAYLEIGH WIDGET'
...
; FOR SITE AND SENSOR
    ...
    UPDATE_PROC:
    IF DHMI_P3_INFO.IVERBOSE EQ 1 THEN PRINT,'DHMI_RAYLEIGH_ARG_SETUP->CHANGE: UPDATING THE PROCESSING VERSION FIELD AND INDEX'
    CSITE = DHMI_P3_INFO.ASITE[DHMI_P3_INFO.ISITE]
...
; FOR SENSOR
    ...
    UPDATE_AER:
    IF DHMI_P3_INFO.IVERBOSE EQ 1 THEN PRINT,'DHMI_RAYLEIGH_ARG_SETUP->CHANGE: UPDATING THE AEROSOL FIELD AND INDEX'
    CSENS = DHMI_P3_INFO.ASENS[DHMI_P3_INFO.ISENS]
...
; SENSOR AND PROC VERSION
    ...
    UPDATE_YEAR:
    IF DHMI_P3_INFO.IVERBOSE EQ 1 THEN PRINT,'DHMI_RAYLEIGH_ARG_SETUP->CHANGE: UPDATING THE YEAR FIELD AND INDEX'
    CSITE = DHMI_P3_INFO.ASITE[DHMI_P3_INFO.ISITE]
    CSENS = DHMI_P3_INFO.ASENS[DHMI_P3_INFO.ISENS]
...
; FIND MAIN DIMITRI FOLDER AND DELIMITER
  ...
  IF KEYWORD_SET(VERBOSE) THEN BEGIN
    PRINT,'DHMI_RAYLEIGH_ARG: STARTING RAYLEIGH HMI ROUTINE'
    IVERBOSE=1
...
; DEFINE THE MAIN WIDGET 
  ...
  DHMI_P3_TLB = WIDGET_BASE(COLUMN=1,TITLE=TOOL+': RAYLEIGH SIMU SETUP',XSIZE=XSIZE,$
...
; RAYLEIGH CAL PARAMETERS
  ...
  DHMI_P3_TLB_3_LBL   = WIDGET_LABEL(DHMI_P3_TLB_3,VALUE='RAYLEIGH SIMU PARAMETERS:', /ALIGN_LEFT)
