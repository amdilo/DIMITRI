...
; DEFINE THE STATISTICAL ARRAYS
  ...
  BADVAL = -999.0
  IFILE_DATE 	  = DBLARR(5);CONTAINS YEAR,MONTH,DAY,DOY,DECIMAL_YEAR
...
; RETRIEVE DATE INFORMATION --MOVE TO EACH DIRECTION; SET DATE/TIME AS FIRST SEQUENCE 
    ...
    IFILE_DATE[4] =  FLOAT(IFILE_DATE[0])+DOUBLE(IFILE_DATE[3]/DIY)+TTIME ;BAH BUG retrieve decimal_year for DEC 31 to JAN 01 TBD
...
; APPLY CLOUD SCREENING
       ...
       CLOUD_MASK = DIMITRI_CLOUD_SCREENING(SENSOR,PAR_SITE_TYPE,CS_RHO,CS_GEO,'GLOBCARBON_P',VERBOSE=VERBOSE)
...
; CALCULATE PERCENTAGE CLOUD
       ...
       CS_PARASOL   = FLOAT(NUM_CS)/FLOAT(NUM_PIX)
       IF CS_PARASOL GT TEMP_AUTO_CS THEN TEMP_AUTO_CS = DOUBLE(CS_PARASOL)
