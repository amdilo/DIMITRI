...
;----------------------------------------
; SELECT DATA UPON ROI AND CLOUD CRITERIA
...
; COMPUTE MARINE SIGNAL WRT CHLOROPHYLL
  ...
  ;RHOW_REF = !PI*0.5287/3.*R0_REF ; rhow =PI*Rgoth/Q*R(0-)
  RHOW_REF = 0.5287*R0_REF ; rhow =PI*Rgoth/Q*R(0-)
...
;----------------------------------------
; COMPUTE DIRECT TRANSMITTANCE AT BREF

  TAUR_REF = TAUR_HT74(WAVREF) 
  TD_REF = EXP(-(TAUA_REF+TAUR_REF)*AIR_MASS)
...
;----------------------------------------
; COMPUTE RHOPATH OVER RHOR AT BREF

     XC_REF = COMPUTE_XC(WIND_SPEED, RAA, VZA, SZA, TAUA_REF, BREF, /COS)

;----------------------------------------
; COMPUTE TOTAL (DIRECT+DIFFUSE) TRANSMITTANCE
...
; COMPUTE GLINT FROM BREF SIGNAL
     ...
     RHO_G = (MI_REF_OZ - RHOR_REF*XC_REF - TRANS_REF*RHOW_REF)/ TD_REF
...
;----------------------------------------
; COMPUTE DIRECT TRANSMITTANCE

     TAUR_K = TAUR_HT74(WAVK) 
     TD_K = EXP(-(TAUA_K+TAUR_K)*AIR_MASS)

;----------------------------------------
; COMPUTE TOTAL TOA SIGNAL
     
     I_K     =  RHOR_K*XC_K + TRANS_K*RHOW_K + TD_K*RHO_G
...
; 1% MAX ACCORDING TO HAGOLLE ET AL 1999
   ...
   IF BK EQ B865 THEN TAUA_865_CHECK = INVERSE_XC(WIND_SPEED, RAA, VZA, SZA,(MI_K_OZ-TRANS_K*RHOW_K-TD_K*RHO_G)/RHOR_K,BK, /COS)
