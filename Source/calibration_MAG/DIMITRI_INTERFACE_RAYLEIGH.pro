...
;*
;* NAME:
;*      DIMITRI_INTERFACE_RAYLEIGH       
;* 
;* PURPOSE:
;*      INTERFACES BETWEEN THE DIMITRI HMI AND THE PRODUCT INGESTION ROUTINES
;* 
;* CALLING SEQUENCE:
;*      RES = DIMITRI_INTERFACE_RAYLEIGH(SITE_NAME,SENSOR,PROC_VERSION)      
;* 
;* INPUTS:
...
FUNCTION DIMITRI_INTERFACE_RAYLEIGH,SITE_NAME,SENSOR,PROC_VERSION,ALL=ALL,YEAR=YEAR,VERBOSE=VERBOSE

  DEBUG_MODE=0       ; SET TO 1 IF WANT TO DEBUG THIS PROCEDURE
  
  FCT_NAME = 'DIMITRI_INTERFACE_RAYLEIGH'
  ...
  FOR IDX_FILE=0, NB_FILES-1 DO BEGIN
    ...
    FOR NUM_DIR=0, NB_DIRECTIONS-1 DO BEGIN
      ...
      ;----------------------
			...
			TAU_ESTIM = INTERP5D(RAYLEIGH_LUT_NIR_STRUCT.THETA_S, RAYLEIGH_LUT_NIR_STRUCT.THETA_V, $
													 RAYLEIGH_LUT_NIR_STRUCT.DPHI, RAYLEIGH_LUT_NIR_STRUCT.WV, $
													 RAYLEIGH_LUT_NIR_STRUCT.REFL_TOA, RAYLEIGH_LUT_NIR_STRUCT.TAU, $
													 THETA_S_VLD, THETA_V_VLD, DPHI_VLD, WATERVAPOUR_VLD, RTOA_TG_RATIO_NIR, $
													 TSI_NIR, TVI_NIR, DPI_NIR, WVI_NIR, ROR, /INVERSE_Z)
      ...
      FOR NUM_BAND=0, NB_BANDS_SENSOR_PROCESS-1 DO BEGIN
        ...
        ; CHECK IF CURRENT BAND IS NIR BAND
         	...
         	IF DEBUG_MODE EQ 1 THEN BEGIN
						...
						NIR_ESTIM = INTERP5D(THETA_S_NIR_LUT, THETA_V_NIR_LUT, DPHI_NIR_LUT, WV_NIR_LUT, TAU_NIR_LUT, LUT_NIR, $
        ...
        ENDIF 
				...
				RTOA_TG_RATIO_ESTIM = INTERP6D(RAYLEIGH_LUT_STRUCT.THETA_S, RAYLEIGH_LUT_STRUCT.THETA_V, $
														 RAYLEIGH_LUT_STRUCT.DPHI, RAYLEIGH_LUT_STRUCT.WV, $
														 RAYLEIGH_LUT_STRUCT.TAU, RAYLEIGH_LUT_STRUCT.CHLOROPHYLL, $
														 RAYLEIGH_LUT_STRUCT.REFL_TOA(*,IDX_LUT), $
