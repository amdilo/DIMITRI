...
;*      21 JAN 2015 - B ALHAMMOUD  - DIMITRI-3 V1.1
;*      16 FEB 2015 - B ALHAMMOUD  - ADDED DIMITRI_VERSION AS VARIABLE
;*      02 JUN 2015 - NCG / MAGELLIUM - UPDATE DATABASE FIELDS WITH DIMITRI V4 SPECIFICATIONS 
;*                                      GET CLOUD DATABASE CSV FILES FROM INGESTION OUTPUT FOLDER INSTEAD OF INPUT FOLDER
;*                                      COMPUTE DECIMAL YEAR WITH DEDICATED FUNCTION 'DATETIME_TO_DECIMAL_YEAR'
;*
;* VALIDATION HISTORY:
...
; CONVERT START AND STOP DATES IN DECIMAL YEAR
  ...
  DATES = [STARTD,STOPD]
  FOR I=0, 1 DO BEGIN
     YEAR  = FIX(STRMID(DATES[I],0,4))
     MONTH = FIX(STRMID(DATES[I],4,2))
     DAY   = FIX(STRMID(DATES[I],6,2))
...
;  YEAR  = FIX(STRMID(STARTD,0,4))
;  MONTH = FIX(STRMID(STARTD,4,2))
;  DAY   = FIX(STRMID(STARTD,6,2))
;  DEC_DATES[0] = DATETIME_TO_DECIMAL_YEAR(YEAR,MONTH,DAY,0,0,0,VERBOSE=VERBOSE)
;  YEAR  = FIX(STRMID(STOPD,0,4))
;  MONTH = FIX(STRMID(STOPD,4,2))
;  DAY   = FIX(STRMID(STOPD,6,2))
;  DEC_DATES[1] = DATETIME_TO_DECIMAL_YEAR(YEAR,MONTH,DAY,23,59,59,VERBOSE=VERBOSE)
...
; UPDATE CURRENT_BUTTON WITH SELECTION
  ...
  MAIN_INPUT = GET_DIMITRI_LOCATION('INGESTION_OUTPUT')
...
; GET AVAILABLE BAND AND WAVREF
    ...
    SENSOR_INFO = GET_SENSOR_BAND_INFO(CSENS)
    NBAND = SENSOR_INFO.NB_BAND
    DHMI_BRDFCS_INFO.AWAV[0:NBAND-1] = SENSOR_INFO.BAND_WAVELENGTH
...
; SENSOR AND PROC VERSION
    ...
    ID=WHERE(STRMATCH(DHMI_DB_DATA.SITE_NAME,CSITE) AND STRMATCH(DHMI_DB_DATA.SENSOR,CSENS) AND STRMATCH(DHMI_DB_DATA.PROCESSING_VERSION,CPROC), NID)
...
; GET AVAILABLE BAND
  ...
  ABAND = INDGEN(BB_MAX)
  AWAV  = FLTARR(BB_MAX)
  SENSOR_INFO = GET_SENSOR_BAND_INFO(CSENS, VERBOSE=VERBOSE)
  NBAND = SENSOR_INFO.NB_BAND
  AWAV[0:NBAND-1] = SENSOR_INFO.BAND_WAVELENGTH
  IBAND = 0
  BAND  = ABAND[IBAND]
  WAV   = AWAV[IBAND]
...
; DEFINE THE MAIN WIDGET 
  ...
  DIMITRI_VERSION = GET_DIMITRI_LOCATION('TOOL')
  DHMI_BRDFCS_TLB = WIDGET_BASE(COLUMN=1,TITLE=DIMITRI_VERSION+': BRDF CLOUD SCREENING SETUP',XSIZE=XSIZE,$
