...
; RETRIEVE DATE INFORMATION
   ...
   IFILE_DATE[0] = STRMID(IFILES[IN_FNAME],STR_POS+14,4)+0.
   IFILE_DATE[1] = STRMID(IFILES[IN_FNAME],STR_POS+18,2)+0.
   IFILE_DATE[2] = STRMID(IFILES[IN_FNAME],STR_POS+20,2)+0.
   ...
   IFILE_DATE[4] = double(IFILE_DATE[0])+double(DOUBLE(IFILE_DATE[3])/DIY)+double(TTIME)
...
; COMPUTE TOA REFLECTANCE FOR EACH BAND 
       ...
       FOR IN_BAND=0,NB_BANDS-1 DO BEGIN
          ...
          L_TOA_REF=GET_AATSR_L1B_REFLECTANCE(IFILES[IN_FNAME],IN_BAND,AATSR_DIRC[IN_DIR],ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE)
...
; COMPUTE DRIFT CORRECTION
          ...
          IF IN_BAND LE 3 THEN BEGIN
            ...
            TEMP_REF = ROI_AVG_TOA_REF[IN_BAND]
...
; REMOVE CURRENT CORRECTIONS, APPLY NEW CORRECTIONS AND STORE DATA
            ...
            ROI_AVG_TOA_REF[IN_BAND] = TEMP_REF
...
;---------------------------------------
; CORRECT ALL PIXELS WITH SAME FACTOR
  
            SF = TEMP_REF/MEAN(TOA_REF[IN_BAND,VALID])
            CS_RHO[*,IN_BAND] = SF*TOA_REF[IN_BAND,*]
            TOA_REF[IN_BAND,*] = SF*TOA_REF[IN_BAND,*]
...
; APPLY CLOUD SCREENING
       ...
       CLOUD_MASK = DIMITRI_CLOUD_SCREENING(SENSOR,ATS_SITE_TYPE,CS_RHO,0,'LCCA',VERBOSE=VERBOSE)
...
; CALCULATE PERCENTAGE CLOUD
       ...
       CS_AATSR     = FLOAT(NUM_CS)/FLOAT(NUM_PIX)

       IF CS_AATSR GT TEMP_AUTO_CS THEN TEMP_AUTO_CS = DOUBLE(CS_AATSR)
