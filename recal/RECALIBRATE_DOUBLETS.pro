...
; ACCEPTABLE DATA
    ...
    ENDIF ELSE BEGIN
      IF KEYWORD_SET(VERBOSE) THEN PRINT, 'DOUBLET_EXTRACTION: ERROR DURING DOUBLET EXTRACTION, NO GOOD DATES FOUND FOR SENSOR ',SENS[RCALS]
      RETURN,-1
    ENDELSE
...
; GET THE AUX AND PRDNAMES INFO
  ...
  FOR ITT = 0l,N_ELEMENTS(SENSOR_TOA_REF[0,*])-1 DO BEGIN
    RES = WHERE(ABS(GD_REF_DATA[0,*]-SENSOR_TOA_REF[0,ITT]) LT 0.000001 AND $
                ABS(GD_REF_DATA[1,*]-SENSOR_TOA_REF[1,ITT]) LT 0.001,COUNT)
    IF COUNT EQ 0 THEN SENSOR_TOA_REF[0,ITT]=-999.
  ENDFOR
...
;---------------------------------------  
; GET THE PRDNAMES INFO
  
  FOR ITT = 0l,N_ELEMENTS(SENSOR_TOA_REF[0,*])-1 DO BEGIN
    RES = WHERE(ABS(GD_CAL_DATA[0,*]-SENSOR_TOA_REF[0,ITT]) LT 0.000001 AND $
                ABS(GD_CAL_DATA[1,*]-SENSOR_TOA_REF[1,ITT]) LT 0.001,COUNT)
    IF COUNT EQ 0 THEN SENSOR_TOA_REF[0,ITT]=-999.
  ENDFOR
...
; AND REF_SENSOR
  ...
  CAL_DATA[NUM_NON_REF:NUM_NON_REF+2*NBANDS_CAL-1,*] = -1.0
...
; RESTORE CALIBRATION COEFICIENTS AND APPLY
    ...
    IF VCOUNT GT 0 THEN BEGIN
      CAL_DATA[NUM_NON_REF+RC_BAND,RES] = -1.0
      CAL_DATA[NUM_NON_REF+NBANDS_CAL+RC_BAND,RES] = -1.0
    ENDIF
