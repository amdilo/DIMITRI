...
; DRAW A LIGHT RED BOX AROUND ROI PIXELS
    ...
    TOL=0.01
    QL_ROI_IDX = WHERE( $
           ( (ABS(QL_GEO_LAT -ICOORDS[0]) LE TOL OR ABS(QL_GEO_LAT -ICOORDS[1]) LE TOL ) AND QL_GEO_LON LT ICOORDS[2] AND QL_GEO_LON GT ICOORDS[3] ) OR $
           ( (ABS(QL_GEO_LON -ICOORDS[2]) LE TOL OR ABS(QL_GEO_LON -ICOORDS[3]) LE TOL ) AND QL_GEO_LAT LT ICOORDS[0] AND QL_GEO_LAT GT ICOORDS[1] ) )
