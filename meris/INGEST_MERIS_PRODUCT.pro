...
; DEFINE OUTPUT
  ...
  NCDF_FILENAME = STRING(INPUT_FOLDER+DELIM+'Site_'+IREGION+DELIM+SENSOR+DELIM+'Proc_'+IPROC+DELIM+SENSOR+'.nc')
...
; ADD DATA TO NETCDF OUTPUT STRUCTURE
  ...
  NCDF_OUT = GET_DIMITRI_EXTRACT_NCDF_STRUCTURE(NB_FILES,NB_BANDS)
  ...
  NCDF_OUT.ATT_NBANDS = STRTRIM(STRING(NUM_BANDS,2))
...
NCDF_OUT.VAR_PTIME[IN_FNAME]  = DB_DATA.YEAR[IN_FNAME]+DB_DATA.MONTH[IN_FNAME]+DB_DATA.DAY[IN_FNAME]+' '+date_hr+':'+date_min+':'+date_sec
...
; SAVE DATA TO NETCDF FILE
  ...
  RES = DIMITRI_INTERFACE_EXTRACT_TOA_NCDF(NCDF_OUT,NCDF_FILENAME,NB_FILES,NB_BANDS)  
