...
; YEAR VALUES
  ...
  IF SITE   EQ 'ALL' THEN CSITE   ='*' ELSE CSITE   = SITE
  IF SENSOR EQ 'ALL' THEN CSENSOR ='*' ELSE CSENSOR = SENSOR
  IF PROCV  EQ 'ALL' THEN CPROCV  ='*' ELSE CPROCV  = PROCV
...
; GET AVAILABLE SENSORS WITHIN REGION
    ...
    IF DHMI_CS_SETUP_INFO.ASITE[DHMI_CS_SETUP_INFO.ISITE] EQ 'ALL' THEN CSITE = '*' $
      ELSE CSITE = DHMI_CS_SETUP_INFO.ASITE[DHMI_CS_SETUP_INFO.ISITE]
     
    TEMP = DHMI_DB_DATA.SENSOR[WHERE(STRMATCH(DHMI_DB_DATA.REGION,CSITE))]
    TEMP = TEMP[UNIQ(TEMP,SORT(TEMP))]
    DHMI_CS_SETUP_INFO.ASENS[0:N_ELEMENTS(TEMP)] = [TEMP,'ALL']
...
; FOR SITE AND SENSOR
    ...
    IF DHMI_CS_SETUP_INFO.ASITE[DHMI_CS_SETUP_INFO.ISITE] EQ 'ALL' THEN CSITE = '*' $
      ELSE CSITE = DHMI_CS_SETUP_INFO.ASITE[DHMI_CS_SETUP_INFO.ISITE]
    IF DHMI_CS_SETUP_INFO.ASENS[DHMI_CS_SETUP_INFO.ISENS] EQ 'ALL' THEN CSENS = '*' $
      ELSE CSENS = DHMI_CS_SETUP_INFO.ASENS[DHMI_CS_SETUP_INFO.ISENS]   
  
    TEMP = DHMI_DB_DATA.PROCESSING_VERSION[WHERE($
                                                 STRMATCH(DHMI_DB_DATA.REGION,CSITE) AND $
                                                 STRMATCH(DHMI_DB_DATA.SENSOR,CSENS))]
    TEMP = TEMP[UNIQ(TEMP,SORT(TEMP))]
    DHMI_CS_SETUP_INFO.APROC[0:N_ELEMENTS(TEMP)] = [TEMP,'ALL']
...
; SENSOR AND PROC VERSION
    ...
    IF DHMI_CS_SETUP_INFO.ASENS[DHMI_CS_SETUP_INFO.ISENS] EQ 'ALL' THEN CSENS = '*' $
      ELSE CSENS = DHMI_CS_SETUP_INFO.ASENS[DHMI_CS_SETUP_INFO.ISENS]   
    IF DHMI_CS_SETUP_INFO.APROC[DHMI_CS_SETUP_INFO.IPROC] EQ 'ALL' THEN CPROC = '*' $
      ELSE CPROC = DHMI_CS_SETUP_INFO.APROC[DHMI_CS_SETUP_INFO.IPROC]
...
; AVAILABLE SENSORS
  ...
  ASENS = MAKE_ARRAY(N_ELEMENTS(USENSS)+1,/STRING,VALUE='')
  APROC = MAKE_ARRAY(N_ELEMENTS(UPROCV)+1,/STRING,VALUE='')
  ...
  TEMP   = TEMP[UNIQ(TEMP,SORT(TEMP))]
  ASENS[0:N_ELEMENTS(TEMP)] = [TEMP,'ALL']
...
; GET AVAILABLE PROCESSING VERSIONS
  ...
  TEMP    = TEMP[UNIQ(TEMP,SORT(TEMP))]
  APROC[0:N_ELEMENTS(TEMP)] = [TEMP,'ALL']
