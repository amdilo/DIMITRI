...
; DRAW A LIGHT RED BOX AROUND ROI PIXELS
    ...
    TOL=0.01
    QL_ROI_IDX = WHERE( $
           ( (ABS(LAT -ICOORDS[0]) LE TOL OR ABS(LAT -ICOORDS[1]) LE TOL ) AND LON LT ICOORDS[2] AND LON GT ICOORDS[3] ) OR $
           ( (ABS(LON -ICOORDS[2]) LE TOL OR ABS(LON -ICOORDS[3]) LE TOL ) AND LAT LT ICOORDS[0] AND LAT GT ICOORDS[1] ) )
