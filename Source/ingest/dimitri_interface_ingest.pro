...
;*      02 FEB 2015 - NCG / MAGELLIUM - DETECT SUNGLINT CASE ON SITE_NAME 
;*
;* VALIDATION HISTORY:
...
; POPULATE ARRAY WITH INFORMATION
  ...
  FOR IN_FILE = 0l,NB_FILES-1 DO BEGIN
    ...
    IF KEYWORD_SET(VERBOSE) THEN PRINT, 'DIM_INTER INGEST: IN PROCESS : ' + FILE_RESULT[IN_FILE] + ' ' + STRING(IN_FILE) + '/' +  STRING(NB_FILES)
    ...
    IF (DB_CHECK EQ 1) AND N_TAGS(DB_DATA) GT 0 THEN BEGIN
      RES = WHERE(STRCMP(DB_DATA.L1_FILENAME,FILE_ARRAY[1,IN_FILE]) EQ 1 AND STRCMP(DB_DATA.SITE_NAME,INGEST_REGION,/FOLD_CASE) EQ 1)
...
; LOOP OVER ALL COMBINATIONS AND INGEST DATA
  ...
  FOR USITE = 0,N_ELEMENTS(UNIQ_SITE)-1 DO BEGIN
    ...
    CURRENT_SITE = UNIQ_SITE[USITE]  
    IF STRCMP(STRUPCASE(CURRENT_SITE), 'SUNGLINT') EQ 1 THEN BEGIN
      INGEST_SUNGLINT = 1
      ICOORDS = [90.,-90.,180.0,-180.0]
    ENDIF ELSE BEGIN
      INGEST_SUNGLINT = 0
      ICOORDS = GET_SITE_COORDINATES(CURRENT_SITE,SITE_FILE,VERBOSE=VERBOSE)     
    ENDELSE  
...
; FIND ALL MATCHING NEW PRODUCTS  
        ...
        'AATSR'       : RES = INGEST_AATSR_PRODUCT(IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
                                                  ENDIAN_SZE=ENDIAN_SIZE,COLOUR_TABLE=COLOUR_TABLE,$
                                                  PLOT_XSIZE=PLOT_XSIZE,PLOT_YSIZE=PLOT_YSIZE,VERBOSE=VERBOSE)

        'ATSR2'       : RES = INGEST_ATSR2_PRODUCT(IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
                                                  ENDIAN_SZE=ENDIAN_SIZE,COLOUR_TABLE=COLOUR_TABLE,$
                                                  PLOT_XSIZE=PLOT_XSIZE,PLOT_YSIZE=PLOT_YSIZE,VERBOSE=VERBOSE)
 
        'MERIS'       : RES = INGEST_MERIS_PRODUCT(IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
                                                  ENDIAN_SZE=ENDIAN_SIZE,COLOUR_TABLE=COLOUR_TABLE,$
                                                  PLOT_XSIZE=PLOT_XSIZE,PLOT_YSIZE=PLOT_YSIZE,VERBOSE=VERBOSE)
 
        'MODISA'      : BEGIN
        
;                        RES = INGEST_MODISA_PRODUCT(IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
;                                                  COLOUR_TABLE=COLOUR_TABLE,$
;                                                  PLOT_XSIZE=PLOT_XSIZE,PLOT_YSIZE=PLOT_YSIZE,VERBOSE=VERBOSE)
        
                        ; HISTORY BEHIND SWITCH BETWEEN LINUX AND WINDOWS NOT UNDERSTOOD, (see TEMP_REF CHANNEL USE) (NO ENDIANESS ISSUE ON FIRST SIGHT) / TODO : ASK TO ARGANS HISTORY BEHIND
                        
                        CASE STRUPCASE(!VERSION.OS_FAMILY) OF
                        'WINDOWS':  RES = INGEST_MODISA_PRODUCT(IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
                                                  COLOUR_TABLE=COLOUR_TABLE,$
                                                  PLOT_XSIZE=PLOT_XSIZE,PLOT_YSIZE=PLOT_YSIZE,VERBOSE=VERBOSE)
                        'UNIX':     RES = INGEST_MODISA_PRODUCT_LINUX(IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
                                                  COLOUR_TABLE=COLOUR_TABLE,$
                                                  PLOT_XSIZE=PLOT_XSIZE,PLOT_YSIZE=PLOT_YSIZE,VERBOSE=VERBOSE)
                        ENDCASE
                        END

        'PARASOL'     : RES = INGEST_PARASOL_PRODUCT(IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
                                                  ENDIAN_SZE=ENDIAN_SIZE,COLOUR_TABLE=COLOUR_TABLE,$
                                                  PLOT_XSIZE=PLOT_XSIZE,PLOT_YSIZE=PLOT_YSIZE,VERBOSE=VERBOSE)

        'VEGETATION'  : RES = INGEST_VEGETATION_PRODUCT(IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
                                                  COLOUR_TABLE=COLOUR_TABLE,$
                                                  PLOT_XSIZE=PLOT_XSIZE,PLOT_YSIZE=PLOT_YSIZE,VERBOSE=VERBOSE)
        ENDCASE
