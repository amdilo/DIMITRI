...
; DEFINE OUTPUT FILENAMES BASED ON NCDF_FILE
  ...
  TITLE_BASE_FIELDS = STRSPLIT(FILE_BASENAME(OUTPUT_BASE),'_', COUNT=COUNT, /EXTRACT)
  TITLE_BASE = TITLE_BASE_FIELDS[0] + ' - ' + TITLE_BASE_FIELDS[1] + ' - ' + STRJOIN(TITLE_BASE_FIELDS[3:COUNT-1],' ') 
...
; FIND ONLY THE GOOD DATA
    ...
    ; EXTRACT_TOA_NCDF.VAR_DTIME[IN_FNAME]
    ; EXTRACT_TOA_NCDF.VAR_RHOMU[IN_BAND,IN_FNAME,IN_DIR]
    ;EXTRACT_TOA_NCDF.VAR_PIX[IN_BAND,IN_FNAME,IN_DIR]
...
; PLOT REFLECTANCE EVOLUTION AGAINST DOY
    ...
    IF KEYWORD_SET(VERBOSE) THEN BEGIN
      IF NB_DIR GT 1 THEN PRINT, FCT_NAME + ' - PLOTTING TOA REFLECTANCE EVOLUTION AGAINST DOY, DIRECTION: ',DIR_STR[IN_DIR]  $
                     ELSE PRINT, FCT_NAME + ' - PLOTTING TOA REFLECTANCE EVOLUTION AGAINST DOY'
    ENDIF
 
    IF NB_DIR GT 1 THEN BEGIN
      OUTPUT_IMAGE = STRING(OUTPUT_BASE+'_'+DIR_STR[IN_DIR]+'_TOA_EVOLUTION.jpg')
      TITLE  = 'TOA Reflectance Evolution - '+DIR_STR[IN_DIR]+' ('+TITLE_BASE+')'
    ENDIF ELSE BEGIN
      OUTPUT_IMAGE = STRING(OUTPUT_BASE+'_TOA_EVOLUTION.jpg')
      TITLE  =  'TOA Reflectance Evolution ('+TITLE_BASE+')'
    ENDELSE
    ...
    FOR BAND=0,NB_BANDS-1 DO BEGIN
      ...
      IF COUNT GT 0 THEN BEGIN
        ...
        ; GET FIRST INDEXES DATE
        XMIN = MIN(EXTRACT_TOA_NCDF.VAR_DTIME[TT], FIRST_IDX)
        
        ; GET MIN/MAX VALUE
        YMIN = MIN(100.0*(L1B_REF_DIR[BAND,TT]-L1B_REF_DIR[BAND,FIRST_IDX])/L1B_REF_DIR[BAND,FIRST_IDX], MAX=YMAX)
        IF N_ELEMENTS(YMIN_RANGE) EQ 0 THEN BEGIN
          YMIN_RANGE = YMIN
          YMAX_RANGE = YMAX
        ENDIF ELSE BEGIN
          YMIN_RANGE = MIN( [ YMIN_RANGE, YMIN ] )
          YMAX_RANGE = MAX( [ YMAX_RANGE, YMAX ] )
        ENDELSE
    ...
    FOR BAND=0,NB_BANDS-1 DO BEGIN
      ...
      IF COUNT GT 0 THEN BEGIN
        ...
        IF FIRST_STEP THEN BEGIN
          ; GRAPH INITIALISATION
          PLOT,EXTRACT_TOA_NCDF.VAR_DTIME[TT],100.0*(L1B_REF_DIR[BAND,TT]-L1B_REF_DIR[BAND,FIRST_IDX])/L1B_REF_DIR[BAND,FIRST_IDX],/NODATA,$
          COLOR = 0, BACKGROUND = 255,$
          YMARGIN = [4,3], $  
          TITLE = TITLE, $
        ...
        OPLOT, EXTRACT_TOA_NCDF.VAR_DTIME[TT], 100.0*(L1B_REF_DIR[BAND,TT]-L1B_REF_DIR[BAND,FIRST_IDX])/L1B_REF_DIR[BAND,FIRST_IDX], COLOR = 250.*(BAND+0.5)/FLOAT(NB_BANDS)
      
      ENDIF
    ENDFOR

    TEMP = TVRD(/TRUE)
    IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ' - EVOLUTION OUTPUT AT ',OUTPUT_IMAGE
    WRITE_JPEG,OUTPUT_IMAGE,TEMP,TRUE=1,QUALITY=100
    ERASE 
...
;   NCDF_OUT.VAR_VZA[IN_DIR,IN_FNAME]
