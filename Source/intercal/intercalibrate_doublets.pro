...
;* 
;* CALLING SEQUENCE:
;*      RES = INTERCALIBRATE_DOUBLETS(OUTPUT_FOLDER,ID_REGION,SENSOR1,PROV_VER1,BID1,SENSOR2,BID2,PROV_VER2)      
;* 
;* INPUTS:
...
;*        21 JUL 2011 - C KENT    - ADDED POLYNOMIAL COVARIANCE MATRIX TO CSV OUTPUT
;*        20 FEB 2014 - C MAZERAN - ADDED SBAF CORRECTION
;*
;* VALIDATION HISTORY:
...
; AND SYSTEMATIC AT 2%, NOT INCLUDING SBAF UNCERTAINTY YET
...
;----------------------------------------
; APPLY SPECTRAL BAND ADJUSTMENT FACTOR

  SBAF = SBAF(ID_REGION,SENSOR1,BID1,REFORM(S1_DATA),SENSOR2,BID2)
  IF SBAF[0] EQ -1 THEN BEGIN
    PRINT,'INTERCALIBRATE_DOUBLETS: ERROR IN SBAF CORRECTION'
    RETURN,-1
  ENDIF
  S2_DATA[0,*] = S2_DATA[0,*]/SBAF[0]
  
;----------------------------------------
; ADD SBAF SYSTEMATIC AND RANDOM UNCERTAINTY 

  UNCERT_ARRAY[0] = SEN_UNCERTS + SBAF[1]
  UNCERT_ARRAY[1] = SQRT(SEN_UNCERTR*SEN_UNCERTR + SBAF[2]*SBAF[2])
  UNCERT_ARRAY[2] = 0.  
...
; FIT AND FITTED POINTS
  ...
  IF UNCERT_ARRAY[1] LT 3*(RMSE) THEN BEGIN
