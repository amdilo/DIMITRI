...
FUNCTION INGEST_MODISA_PRODUCT,IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,INGEST_SUNGLINT=INGEST_SUNGLINT,$
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ' - STARTING INGESTION LOOP ON MERIS PRODUCTS'
    ...
    DATE_SEC = STRING(FORMAT="(I2.2)", STRTRIM(TEMP_DATE.SECOND,1))
    ...
    NCDF_INGEST_STRUCT.GLOBAL_ATT.L1_FILENAME = DB_DATA.L1_FILENAME[IN_FNAME] 
		...
		IF N_ELEMENTS(NO_QUICKLOOK) EQ 0 THEN BEGIN
			...
			IF FIX(CFIG_DATA.(1)[3]) EQ 1 THEN QL_STATUS =  GET_MODISA_QUICKLOOK(IFILES[IN_FNAME],OUTPUT_QL_FILENAME,/ROI,/RGB,ICOORDS=ICOORDS,VERBOSE=VERBOSE) $
				ELSE QL_STATUS =  GET_MODISA_QUICKLOOK(IFILES[IN_FNAME],OUTPUT_QL_FILENAME,/ROI,ICOORDS=ICOORDS,VERBOSE=VERBOSE)
