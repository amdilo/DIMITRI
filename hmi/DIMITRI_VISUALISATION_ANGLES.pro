...
;  RES = WHERE(180.0-TMP_REF_VAA LT 0.0)
;  IF RES[0] GT -1 THEN TMP_REF_VAA[RES] = TMP_REF_VAA[RES]-360.0
...
; GET SENSOR CONFIGURATION FROM FILENAME
    ...
    FOR J=0,4 DO BEGIN
      ANGL_DATA[0:TEMP_ELEMENTS-1,I+1,J] = DOUBLE(ED_SENSOR2_SENSOR1[J,*])
      IF J EQ 2 THEN BEGIN
        TVAA  = DOUBLE(ED_SENSOR2_SENSOR1[J,*])
        RES   = WHERE(180.0-TVAA LT 0.0)
        IF RES[0] GT -1 THEN TVAA[RES] = TVAA[RES]-360.0
        ANGL_DATA[0:TEMP_ELEMENTS-1,I+1,J]=TVAA
      ENDIF
    ENDFOR 
  ENDFOR
