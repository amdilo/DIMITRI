...
;-----------------------------------------
; SET SENSOR RANDOM AND SYSTEMATIC UNCERTAINTY AT 3% 
...
; APPLY SBAF CORRECTION
    ...
    BINDEX   = CONVERT_WAVELENGTH_TO_DINDEX(TEMPB[0])
    REF_BAND = GET_SENSOR_BAND_INDEX(TEMPR_SENSOR,BINDEX+1)

    SBAF = SBAF(RC_REGION,REF_SENSOR,REF_BAND,GD_REF_DATA[NUM_NON_REF+REF_BAND,*],CAL_SENSOR,RC_BAND)
    IF SBAF[0] EQ -1 THEN BEGIN
      PRINT,'RECALIBRATE_DATA: ERROR IN SBAF CORRECTION'
      RETURN,-1
    ENDIF
    CAL_DATA[NUM_NON_REF+RC_BAND,*]  = CAL_DATA[NUM_NON_REF+RC_BAND,*]/SBAF[0]
...
; RESTORE CALIBRATION COEFICIENTS AND APPLY
    ...
    CAL_DATA[NUM_NON_REF+RC_BAND,*]  = CAL_DATA[NUM_NON_REF+RC_BAND, *]*(1.0-BIAS/100.0)
