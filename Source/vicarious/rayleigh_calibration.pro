...
;----------------------------------------
; DEFINE PIXEL CLOUD MASK; 0 BY DEFAULT FOR AVERAGED INPUT
...
; SELECT DATA UPON WIND AND CLOUD MASK CONDITION
  ...
  ID =  WHERE(WIND_SPEED LE WS_MAX AND CLOUD_MASK EQ 0) 
  IF ID[0] EQ -1 THEN BEGIN
    PRINT, FCT_NAME+': WARNING, NO SENSOR DATA FOUND WITH CLEAR SKY OR LOW WIND SPEED, RETURN'
    RETURN, 0
  ENDIF
...
;----------------------------------------
; SAVE VICARIOUS COEFFICIENTS IN PIX MODEL
...
; PLOT AVG VICARIOUS COEF AGAINST DOY
  ...
  FOR K=0, NB_BANDS_VIS-1 DO BEGIN
     ...
     YTITLE = rho+'_obs/'+rho+'_sim('+STRTRIM(FIX(WAVK),2)+')', XTITLE = 'DECIMAL YEAR',  XRANGE = [XMIN,XMAX], YRANGE = [YMIN,YMAX],$
     XTICKFORMAT='((F8.3))'
...
;BAH  PLOT, WAV_VIS, MEAN_COEF, COLOR=0, BACKGROUND=255, PSYM=-8,$
  PLOT, WAV_VIS, MEDI_COEF, COLOR=0, BACKGROUND=255, PSYM=-8,$
  YTITLE = rho+'_obs/'+rho+'_sim', XTITLE = 'WAVELENGTH (NM)', YRANGE = [YMIN, YMAX]
  ERRPLOT, WAV_VIS, MEDI_COEF-0.5*STDD_COEF,MEDI_COEF+0.5*STDD_COEF, COLOR=0  
;BAH  ERRPLOT, WAV_VIS, MEAN_COEF-0.5*STDD_COEF,MEAN_COEF+0.5*STDD_COEF, COLOR=0  

  TEMP = TVRD(TRUE=1)
  JPEG = STRJOIN(['RAYLEIGH_CAL',VIC_REGION,VIC_SENSOR,VIC_PROC_VER],'_')+'_MEDIAN.JPG'
;BAH  JPEG = STRJOIN(['RAYLEIGH_CAL',VIC_REGION,VIC_SENSOR,VIC_PROC_VER],'_')+'_MEAN.JPG'
  OUT_JPEG=FILEPATH(JPEG, ROOT_DIR=OUTPUT_FOLDER)
  WRITE_JPEG,OUT_JPEG,TEMP,TRUE=1,QUALITY=100
  ERASE
