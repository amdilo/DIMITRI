...
;*        01 JUN 2015 - NCG / MAGELLIUM - CHANGE PATH OF REF_TOA.dat FILES TO TEMPORARY FILES 
;*                                CREATED BY EXTRACT_DOUBLETS FUNCTION (REF_TOA.dat FILES REPLACED 
;*                                BY NetCDF FILES IN DIMITRI V4) - ADD COMMON RETURN STATUS
;*
;* VALIDATION HISTORY:
...
; CHECK OFOLDER EXISTS AND IS CORRECT FORMAT
  ...
  IF RES.EXISTS EQ 0 THEN BEGIN
    IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ": OUTPUT FOLDER DOESN'T EXIST"
    RETURN, STATUS_ERROR
  ENDIF
...
; DEFINE INPUT/OUTPUT FILES
  ...
  REF_IFILE = STRING(OUTPUT_FOLDER+DL+'TOA_REF_'+RC_REGION+'_'+REF_SENSOR+'_'+REF_PROC_VER+'.dat')
  CAL_IFILE = STRING(OUTPUT_FOLDER+DL+'TOA_REF_'+RC_REGION+'_'+CAL_SENSOR+'_'+CAL_PROC_VER+'.dat')
...
; DOESN'T EXIST
  ...
  IF RES.EXISTS NE 1 OR RES.DIRECTORY NE 1 THEN BEGIN
    IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ":  RECALIBRATION FOLDER DOESN'T EXIST, CREATING"
...
; CHECKS OFFSET AND PERCENTAGES ARE OK
  ...
  IF  CP_LIMIT  GT 1.0 OR CP_LIMIT  LT 0.0 OR $
    ...
    IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ': ERROR, CLOUD/ROI PERCENTAGES OUT OF RANGE'
    RETURN, STATUS_ERROR
  ENDIF
...
; COMPUTE ROI AREA IN KM^2
  ...
  IF ICOORDS[0] EQ -1 THEN BEGIN
    IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': ERROR, REGION COORDINATES NOT FOUND'
    RETURN, STATUS_ERROR
  ENDIF
...
; READ THE DIMITRI DATABASE
  ...
  IF TEMP.EXISTS EQ 0 THEN BEGIN
    IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ': ERROR, DIMITRI DATABASE FILE DOES NOT EXIST'
    RETURN, STATUS_ERROR
  ENDIF
...
; RESTORE THE SENSOR TOA DATA
  ...
  IF FILE_TEST(REF_IFILE) EQ 0 OR $
     ...
     IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ': ERROR, INPUT DATA NOT FOUND'
     RETURN, STATUS_NODATA
  ENDIF
       
  RESTORE,REF_IFILE
  S1_ADATA = SENSOR_ALL_DATA
  RESTORE,CAL_IFILE
  S2_ADATA = SENSOR_ALL_DATA 
  SENSOR_ALL_DATA=0
...
; SET ALL DATA VARIABLE  
    ...
    IF RES[0] EQ -1 THEN BEGIN
      IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ': ERROR, NO SENSOR DATA FOUND WITHIN PIXEL THRESHOLD'  
      RETURN, STATUS_ERROR
    ENDIF
...
; WITHIN THE CLOUD PERCENTAGE 
    ...
    ; IF AUTO_CS IS LESS THAN PERCENTAGE BUT GREATER THAN -1.0 STORE DECIMAL DATE 
      ...
      IF DB_DATA.AUTO_CS_1_MEAN[RES[I_CS]] LE CP_LIMIT AND DB_DATA.AUTO_CS_1_MEAN[RES[I_CS]] GT -1.0 THEN $
      GD_DATE = [GD_DATE,DB_DATA.DECIMAL_YEAR[RES[I_CS]]]

    ENDFOR;END OF LOOP ON GOOD DATES

    IF N_ELEMENTS(GD_DATE) EQ 1 THEN BEGIN
       IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ': ERROR, NO SENSOR DATA FOUND WITHIN CLOUD THRESHOLD'
       RETURN, STATUS_ERROR
    ENDIF
...
; ACCEPTABLE DATA
    ...
    ENDIF ELSE BEGIN
      IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ': ERROR DURING DOUBLET EXTRACTION, NO GOOD DATES FOUND FOR SENSOR ',SENS[RCALS]
      RETURN, STATUS_ERROR
    ENDELSE
...
; FIND CALIBRATION COEFICIENTS
    ...
    IF STRUPCASE(TEMPB) EQ 'ERROR' THEN BEGIN
      IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': ERROR FINDING WAVELENGTH STRING, GOING TO NEXT BAND'
      GOTO,NEXT_BAND
    ENDIF
    TEMP = STRING(TMP_IFILE+TEMPB+'.dat')
    RES  = FILE_INFO(TEMP)
    IF RES.EXISTS EQ 0 THEN BEGIN
      IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ': NO CALIBRATION COEFICIENTS FOUND, GOING TO NEXT BAND'
      GOTO,NEXT_BAND
    ENDIF
...
; RESTORE CALIBRATION COEFICIENTS AND APPLY
  ...
  IF CAL_COUNTER EQ 0 THEN BEGIN
    IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ': ERROR, NO INTERCAL COEFICIENTS FOUND'
    RETURN, STATUS_NODATA
  ENDIF
...
; CREATE AN OUTPUT CSV FILE CONTAINING ALL DATA 
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT,FCT_NAME + ': SUCCESSFULLY RECALIBRATED DATA'    
  RETURN, STATUS_OK
