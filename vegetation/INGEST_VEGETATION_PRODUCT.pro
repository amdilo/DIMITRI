...
; GET OZONE AND WVAP CONCENTRATIONS
      ...
      IFILE_AUX[0] = 1000.*MEAN(TEMP(ROI_INDEX))
      IFILE_AUX[1] = 1000.*STDEV(TEMP(ROI_INDEX)) 

      TEMP = GET_VEGETATION_WVAP(IFILES[IN_FNAME],VERBOSE=VERBOSE)  
      IFILE_AUX[10] = MEAN(TEMP(ROI_INDEX))
      IFILE_AUX[11] = STDEV(TEMP(ROI_INDEX))     
...
; LOOP OVER EACH BAND
      ...
      VALID = WHERE(TEMP_REF(ROI_INDEX) GT 0.0 AND TEMP_REF(ROI_INDEX) LT 5.0)
