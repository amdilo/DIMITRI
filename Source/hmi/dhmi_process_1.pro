...
;*      12 MAY 2015 - NCG/MAGELLIUM - ADDED OUTPUT STATUS CHECK AFTER DIMITRI_INTERFACE_DOUBLET FUNCTION
;*
;* VALIDATION HISTORY:
...
; AND EXTRACT DOUBLETS
  ...
  FOR DHMI_I=0,NCONFIGS-1 DO BEGIN
    ...
    IF RES EQ 1 THEN AT_LEAST_ONE_CONFIG = 1
  ENDFOR
  
  IF AT_LEAST_ONE_CONFIG EQ 0 THEN BEGIN
   MSG = ['DIMITRI PROCESS 1:','NO VALID CONFIGURATION FOUND']
...
; AND PROCESSING VERSIONS
  ...
  USITES        = DHMI_DB_DATA.SITE_NAME[UNIQ(DHMI_DB_DATA.SITE_NAME,SORT(DHMI_DB_DATA.SITE_NAME))]
...
; GET CONFIGURATIONS AVAILABLE FOR FIRST REFERENCE SENSOR
  ...
  RES       = WHERE(DHMI_DB_DATA.SITE_NAME EQ USITES[0],COUNT)
...
; DEFINE THE MAIN WIDGET 
  ...
  DHMI_P1_TLB = WIDGET_BASE(COLUMN=1,TITLE=GET_DIMITRI_LOCATION('TOOL')+': SENSOR RECAL SETUP',XSIZE=XSIZE,$
...
; AND EXIT BUTTONS
  ...
  DHMI_P1_INFO = {$
                  ...
                  REGION_LIST           : DHMI_DB_DATA.SITE_NAME,$
