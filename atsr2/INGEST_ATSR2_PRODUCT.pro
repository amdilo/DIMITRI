...
;*
;* NAME:
;*      INGEST_ATSR2_PRODUCT       
;* 
;* PURPOSE:
;*      INGESTS ATSR2 L1B DATA INTO DIMITRI DATABASE. MULTIPLE PRODUCT 
...
;* 
;* CALLING SEQUENCE:
;*      RES = INGEST_ATSR2_PRODUCT(IFILES)      
;* 
;* INPUTS:
...
;*        13 DEC 2010 - C KENT    - UPDATED REFLECTANCE CALCULATION SECTION
;*        20 DEC 2010 - C KENT    - UPDATED COMMENTS AND HEADER
...
;*
;* VALIDATION HISTORY:
;*        13 DEC 2010 - C KENT    - WINDOWS 32BIT MACHINE IDL 7.1: COMPILATION AND EXECUTION 
...
FUNCTION INGEST_ATSR2_PRODUCT,IFILES,INPUT_FOLDER=INPUT_FOLDER,ICOORDS=ICOORDS,$
...
; KEYWORD PARAMETER CHECK - NOTE, ASSUMES ALL PRODUCT ARE RELATED TO THE SAME REGION/PROCESSING
  ...
  IF STRCMP(STRING(IFILES[0]),'') THEN BEGIN
    PRINT, 'ATSR2 INGESTION ERROR: NO INPUT FILES PROVIDED, RETURNING...'
    RETURN,-1
  ENDIF  
  ...
  AT2_SITE_TYPE = GET_SITE_TYPE(IREGION,VERBOSE=VERBOSE)
  
  IF N_ELEMENTS(ICOORDS) EQ 0 THEN BEGIN
    PRINT, 'ATSR2 L1B INGEST: NO ROI COORDINATES PROVIDED, USING DEFAULT OF [90.,-90,180.0,-180.0]'
    ICOORDS = [90.,-90.,180.0,-180.0]
  ENDIF
  IF N_ELEMENTS(COLOUR_TABLE) EQ 0 THEN BEGIN
    PRINT, 'ATSR2 L1B INGEST: NO COLOR_TABLE SET, USING DEFAULT OF 39'
    COLOUR_TABLE = CFIG_DATA.(1)[2]
  ENDIF
  IF N_ELEMENTS(PLOT_XSIZE) EQ 0 THEN BEGIN
    PRINT, 'ATSR2 L1B INGEST: PLOT_XSIZE NOT SET, USING DEFAULT OF 700'
    PLOT_XSIZE = CFIG_DATA.(1)[0]
  ENDIF
  IF N_ELEMENTS(PLOT_YSIZE) EQ 0 THEN BEGIN
    PRINT, 'ATSR2 L1B INGEST: PLOT_YSIZE NOT SET, USING DEFAULT OF 400'
...
; IF ENDIAN SIZE NOT PROVIDED THEN GET VALUE
   ...
   IF N_ELEMENTS(ENDIAN_SIZE) EQ 0 THEN BEGIN
    IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: NO ENDIAN SIZE PROVIDED, RETRIEVING...'
...
; DEFINE AATSR SPECIFIC PARAMETERS 
  ...
  NB_BANDS = NB_BANDS[0]
  FNAME_STR = 'AT2_TOA_1'
  ATSR2_DIRC = ['NADIR', 'FWARD']
  NB_DIRECTIONS = N_ELEMENTS(ATSR2_DIRC)
...
; DEFINE THE STATISTICAL ARRAYS
  ...
  IF KEYWORD_SET(VERBOSE) THEN BEGIN
    PRINT, 'ATSR2 L1B INGEST: DEFINITION OF OUTPUT ARRAYS:'
...
; START MAIN LOOP OVER EACH IFILE
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: STARTING INGESTION LOOP ON ATSR2 PRODUCTS'
...
; GENERATE A QUICKLOOK WITH THE ROI OVERLAID
    ...
    IF N_ELEMENTS(NO_QUICKLOOK) EQ 0 THEN BEGIN
      IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: GENERATE QUICKLOOK OF PRODUCT'
        IF FIX(CFIG_DATA.(1)[3]) EQ 1 THEN QL_STATUS =  GET_ATSR2_QUICKLOOK(IFILES[IN_FNAME],/ROI,/RGB,ICOORDS=ICOORDS,QL_QUALITY=QL_QUALITY,ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE) $
          ELSE QL_STATUS =  GET_ATSR2_QUICKLOOK(IFILES[IN_FNAME],/ROI,ICOORDS=ICOORDS,QL_QUALITY=QL_QUALITY,ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE)

      IF KEYWORD_SET(VERBOSE) THEN IF QL_STATUS EQ -1 THEN PRINT, 'ATSR2 L1B INGEST: QUICKLOOK GENERATION FAILED - ',IFILES[IN_FNAME] $
        ELSE PRINT, 'ATSR2 L1B INGEST: QUICKLOOK GENERATION SUCCESS' 
    ENDIF 

;------------------------------------------
; RETRIEVE AUX DATA FILENAMES FOR DB_DATA

	TEMP = GET_ATSR2_AUX_FILES(IFILES[IN_FNAME],VERBOSE=VERBOSE)	
	...
	DB_DATA.AUX_DATA_7[IN_FNAME] = 'NONE' 
	DB_DATA.AUX_DATA_8[IN_FNAME] = 'NONE' 
...
; RETRIEVE DATE INFORMATION
    ...
    IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: RETRIEVING DATE INFORMATION'
    STR_POS = STRPOS(IFILES[IN_FNAME],FNAME_STR, /REVERSE_SEARCH)
    IFILE_DATE[0,2*IN_FNAME:2*IN_FNAME+1]	= STRMID(IFILES[IN_FNAME],STR_POS+14,4)
    IFILE_DATE[1,2*IN_FNAME:2*IN_FNAME+1]	= STRMID(IFILES[IN_FNAME],STR_POS+18,2)
    IFILE_DATE[2,2*IN_FNAME:2*IN_FNAME+1]	= STRMID(IFILES[IN_FNAME],STR_POS+20,2)
    ...
    IFILE_DATE[4,2*IN_FNAME:2*IN_FNAME+1]  = FLOAT(IFILE_DATE[0,2*IN_FNAME])+(DOUBLE(IFILE_DATE[3,2*IN_FNAME])/DIY)+TTIME
...
; RETRIEVE INPUT FILE L1B RADIANCE - BASE PIXEL VALIDITY ON B0
    ...
    IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: RETRIEVING REFLECTANCE INFORMATION'
    IFILE_TOA = GET_ATSR2_L1B_REFLECTANCE(IFILES[IN_FNAME],0,ATSR2_DIRC[IN_DIR],ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE)
    TOA_DIMS = SIZE(IFILE_TOA)
	
;----------------------------------
; RETRIEVE INPUT FILE GEOLOCATION

    IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: RETRIEVING GEOLOCATION INFORMATION'
		IFILE_GEO = GET_ATSR2_LAT_LON(IFILES[IN_FNAME],ATSR2_DIRC[IN_DIR],TOA_DIMS[2],TOA_DIMS[1],VERBOSE=VERBOSE)
    
;------------------------------------------
; RETRIEVE INDEX OF NOMINAL DATA WITHIN ROI

    IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: RETRIEVING INDEX OF PIXELS WITHIN ROI'
		ROI_INDEX = WHERE($
...
; RETRIEVE VIEWING GEOMETRIES
    ...
    IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: RETRIEVING VIEWING GEOMETRIES'
    TEMP = GET_ATSR2_VIEWING_GEOMETRIES(IFILES[IN_FNAME],ATSR2_DIRC[IN_DIR],TOA_DIMS[2],TOA_DIMS[1],VERBOSE=VERBOSE)
...
; COMPUTE TOA REFLECTANCE FOR EACH BAND 
    ...
    IFILE_TOA = 0
    IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: STARTING LOOP OVER EACH BAND'
		
		FOR IN_BAND=0,NB_BANDS-1 DO BEGIN

      IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: RETRIEVING RADIANCE FOR BAND -',IN_BAND
      TOA_REF=GET_ATSR2_L1B_REFLECTANCE(IFILES[IN_FNAME],IN_BAND,ATSR2_DIRC[IN_DIR],ENDIAN_SIZE=ENDIAN_SIZE,VERBOSE=VERBOSE)
...
; STORE REF IN CS ARRAY
      ...
      IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: END OF LOOP ON BAND - ',IN_BAND
		  ENDFOR; END OF BAND ANALYSIS

;----------------------------------
; APPLY CLOUD SCREENING
 
      CS_ATSR2 = DIMITRI_CLOUD_SCREENING(SENSOR,AT2_SITE_TYPE,CS_RHO,0,'LCCA',VERBOSE=VERBOSE)
      IF CS_ATSR2[0] GT TEMP_AUTO_CS THEN TEMP_AUTO_CS = DOUBLE(CS_ATSR2[0])
  ...
  DB_DATA.AUTO_CS[IN_FNAME] = TEMP_AUTO_CS
	...
	IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: END OF LOOP ON PRODUCT'
	ENDFOR; END OF FILE ANALYSIS

;------------------------------------
; AMEND/SAVE DATA TO SAV FILE 

  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: DEFINING OUTPUT ARRAY'
  NUM_NON_REF = 5+12
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: AMMENDING DATA TO OUTPUT SAV FILE'
...
; GENERATE PLOTS WITH NEW TIME SERIES DATA
  ...
  RES = GET_ATSR2_TIMESERIES_PLOTS(OUTPUT_SAV,COLOUR_TABLE=COLOUR_TABLE,PLOT_XSIZE=PLOT_XSIZE,PLOT_YSIZE=PLOT_YSIZE,VERBOSE=VERBOSE)

;------------------------------------
; AMEND DATA TO DATABASE

  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'ATSR2 L1B INGEST: SENDING DATA TO UPDATE DATABASE'
