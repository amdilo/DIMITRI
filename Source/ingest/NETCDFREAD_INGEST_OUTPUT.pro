...
;*      15 JUN 2015 - NCG / MAGELLIUM - ADD DYNAMIC CAST OF GLOBAL ATTRIBUT WITH THE USE OF 'ATT = CALL_FUNCTION(TYPE_NAME, VALUE)'
;*
;* VALIDATION HISTORY:
...
;  ENDIF
  ...
  FOR NUM=0, NB_GLOBAL_ATT-1 DO BEGIN
    TAG_TYPENAME = SIZE(NCDF_INGEST_STRUCT.GLOBAL_ATT.(NUM), /TNAME)
    NCDF_ATTGET, NCID, TAGS_NAME[NUM] , ATTRIBUT , /GLOBAL
;    help, ATTRIBUT, NCDF_INGEST_STRUCT.GLOBAL_ATT.(NUM)   
    NCDF_INGEST_STRUCT.GLOBAL_ATT.(NUM) = CALL_FUNCTION(TAG_TYPENAME, ATTRIBUT)
;    help, NCDF_INGEST_STRUCT.GLOBAL_ATT.(NUM)   
  ENDFOR
