...
;*
;* NAME:
;*      DIMITRI_INTERFACE_SUNGLINT       
;* 
;* PURPOSE:
;*      INTERFACES BETWEEN THE DIMITRI HMI AND THE PRODUCT INGESTION ROUTINES
;* 
;* CALLING SEQUENCE:
;*      RES = DIMITRI_INTERFACE_SUNGLINT(SITE_NAME,SENSOR,PROC_VERSION)      
;* 
;* INPUTS:
...
FUNCTION SUNGLINT_COMMON_PROCESS, NUM_DIR, SENSOR_CONFIG, SUNGLINT_STRUCT, SUNGLINT_LUT_FILE_STRUCT, SMAC_FILE_STRUCT, NCDF_CALIB_STRUCT, VERBOSE=VERBOSE
  ...
  VISR_BAND_IDX = VISR_BAND_IDX[0]
	...
	; REMOVE WIND TEST AND ADD TEST THETA_S/V ANGLES GE 15° (LIMIT VALUE OF THE LUT FILE)
...
;                          AND SUNGLINT_STRUCT.WIND_SPEED[INDX_CS_MASK_CLEAR] LT MAX_WS_SG_CALIBRATION $
  ...
  IDX_VISR_LUT = WHERE(SUNGLINT_LUT_FILE_STRUCT.IDX_VALID_LUT_FILE EQ VISR_BAND_IDX, COUNT)
	WS_ESTIM = INTERP4D(SUNGLINT_LUT_STRUCT.THETA_S, SUNGLINT_LUT_STRUCT.THETA_V, SUNGLINT_LUT_STRUCT.DPHI, $
												SUNGLINT_LUT_STRUCT.REFL_TOA[*,IDX_VISR_LUT], SUNGLINT_LUT_STRUCT.WS, $
												THETA_S_VLD, THETA_V_VLD, DPHI_VLD, RTOA_TG_RATIO_VISR, $ 
												TSI, TVI, DPI, ROR, /INVERSE_Z)
	...
	FOR NUM_BAND=0, NB_BANDS_SENSOR_PROCESS-1 DO BEGIN
		...
		IF IDX_NUM_BAND EQ VISR_BAND_IDX THEN BEGIN
			...
			IF DEBUG_MODE EQ 1 THEN BEGIN
				...
				VISR_ESTIM = INTERP4D(SUNGLINT_LUT_STRUCT.THETA_S, SUNGLINT_LUT_STRUCT.THETA_V, SUNGLINT_LUT_STRUCT.DPHI, $
															SUNGLINT_LUT_STRUCT.WS, SUNGLINT_LUT_STRUCT.REFL_TOA[VISR_BAND_IDX], $
		...
		RTOA_TG_RATIO_ESTIM = INTERP4D(SUNGLINT_LUT_STRUCT.THETA_S, SUNGLINT_LUT_STRUCT.THETA_V, SUNGLINT_LUT_STRUCT.DPHI, $
													SUNGLINT_LUT_STRUCT.WS, SUNGLINT_LUT_STRUCT.REFL_TOA(*,IDX_LUT), $
													THETA_S_VLD, THETA_V_VLD, DPHI_VLD, WS_ESTIM_VLD, $
													TSI, TVI, DPI, WSI)
...
FUNCTION DIMITRI_INTERFACE_SUNGLINT,SITE_NAME,SENSOR,PROC_VERSION,ALL=ALL,YEAR=YEAR,VERBOSE=VERBOSE

  DEBUG_MODE=0       ; SET TO 1 IF WANT TO DEBUG THIS PROCEDURE
  
  FCT_NAME = 'DIMITRI_INTERFACE_SUNGLINT'
