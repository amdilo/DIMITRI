...
;        RTOA = NCDF_INGEST_STRUCT.VARIABLES.REFL_BAND[0:NB_PIXEL_ROI-1, NUM_DIR, IDX_VALID_BANDS[BAND]]
  		...
  		DESERT_STRUCT.OZONE[IDX_STRUCT] = MEAN(NCDF_INGEST_STRUCT.VARIABLES.ERA_OZONE[IDX_CLEAR,NUM_DIR])
			DESERT_STRUCT.WV[IDX_STRUCT]    = MEAN(NCDF_INGEST_STRUCT.VARIABLES.ERA_WATERVAPOUR[IDX_CLEAR,NUM_DIR])
  ...
  ;----------------------
  ; CONSOLIDATE OZONE AND WATERVAPOUR VALUE IF ASKED
  
  IF KEYWORD_SET(CONSOLIDATED) THEN BEGIN

    IF KEYWORD_SET(VERBOSE) THEN PRINT, FCT_NAME + ': CONSOLIDATION OF OZONE AND WATERVAPOUR METEO DATA'
    ...
    IDX_VALID = WHERE(DESERT_STRUCT.VALID EQ 1, COUNT)
    IF COUNT NE 0 THEN BEGIN
      ; RETRIEVE OZONE AND WATERVAPOUR VALUES ATTHE ACQUISITION DATE AND CENTER LAT/LON COORD
      
      LAT_ROI_CENTER_TAB = FLTARR(COUNT)+LAT_ROI_CENTER
      LON_ROI_CENTER_TAB = FLTARR(COUNT)+LON_ROI_CENTER
      
      DESERT_STRUCT.OZONE[IDX_VALID] = READ_ECMWF_INTERIM(DECIMAL_YEAR[IDX_VALID], LAT_ROI_CENTER_TAB, LON_ROI_CENTER_TAB, 'TCO')
      DESERT_STRUCT.WV[IDX_VALID]    = READ_ECMWF_INTERIM(DECIMAL_YEAR[IDX_VALID], LAT_ROI_CENTER_TAB, LON_ROI_CENTER_TAB, 'WV')
    ENDIF
...
;  IDX_VALID=WHERE((GOOD_INDEX EQ 1) AND (SENSOR_L1B_REF[16+SPECTRAL_BAND,*] GT 0) AND (SENSOR_L1B_REF[16+SPECTRAL_BAND,*] LT 5) )
...
;    MATCHING_GEOMETRIES = VALID_RPV_BRDF_GEOMETRY(SENSOR_L1B_REF[1,IDX_VALID], SENSOR_L1B_REF[2,IDX_VALID], SENSOR_L1B_REF[3,IDX_VALID], SENSOR_L1B_REF[4,IDX_VALID], MAX_ANGULAR_DISTANCE, SITE, /VERBOSE)
